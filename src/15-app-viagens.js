          ...
          return React.createElement('button', {
            ...
            mTrips.length === 0
              ...
              : React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 3 } },
                  React.createElement('div', { style: { display: 'flex', gap: 3, flexWrap: 'wrap', marginBottom: 2 } },
                    mTrips.slice(0, 3).map(function(t) { var ti = vgType(t.tipo); return React.createElement('span', { key: t.id, style: { fontSize: 13 } }, ti.emoji); })
                  ),
                  React.createElement('div', { style: { fontSize: 10, fontWeight: 700, color: T.muted } }, mTrips.length + (mTrips.length === 1 ? ' entrada' : ' entradas')),
                  mTrips.some(function(t) { return t.fotos_count > 0; }) && React.createElement('div', { style: { fontSize: 10, color: T.gold } }, '📷 ' + mTrips.reduce(function(s, t) { return s + (t.fotos_count || 0); }, 0))
                )
          );
        })
      ),
