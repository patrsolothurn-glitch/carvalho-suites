...
function FamiliaApp(_ref19) {
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    var allEvsForCell = [].concat(_toConsumableArray(ev.map(function (e) {
      var _dObj = new Date(dStr + 'T00:00:00');
      var _todayM = new Date(); _todayM.setHours(0,0,0,0);
      var isDoneCell = e.arquivado && _dObj < _todayM;
      return { t: e.t, hora: e.hora, color: isDoneCell ? '#2563EB' : e.color, done: isDoneCell };
    })), _toConsumableArray(sh.map(function (s) {
      return { t: s.tipo === 'ferias' ? 'Férias' : s.tipo === 'feriado' ? 'Feriado' : 'Livre', hora: '', color: F.purple, done: false };
    }))).sort(function (a, b) {
      var toMin = function(h) { if (!h) return 9999; var p = h.split(':'); return parseInt(p[0]||0)*60+parseInt(p[1]||0); };
      return toMin(a.hora) - toMin(b.hora);
    });
    var maxShow = 2;
    var visibleEvs = allEvsForCell.slice(0, maxShow);
    var extraCount = allEvsForCell.length - visibleEvs.length;
    return /*#__PURE__*/React.createElement("div", {
      key: d,
      onClick: function onClick() {
        return setSelDay(d);
      ...
      style: {
        ...
        border: hasAny && !isSel ? "1px solid ".concat(((_ev$ = ev[0]) === null || _ev$ === void 0 ? void 0 : _ev$.color) || F.purple, "40") : '1px solid transparent',
        minHeight: 68,
        display: 'flex',
        flexDirection: 'column',
        gap: 2,
      ...
      style: {
        fontSize: 12,
        fontWeight: isSel || hasAny ? 800 : 500,
        color: isSel ? '#fff' : isSun ? F.red : isSat ? F.coral : F.text,
        lineHeight: 1.2,
        textAlign: 'center',
        marginBottom: 1
      }
    }, d), visibleEvs.map(function (e2, ei2) {
      return /*#__PURE__*/React.createElement("div", {
        key: ei2,
        style: {
          fontSize: 8,
          fontWeight: 700,
          lineHeight: 1.25,
          color: isSel ? '#fff' : e2.color,
          background: isSel ? 'rgba(255,255,255,0.2)' : "".concat(e2.color, "18"),
          borderRadius: 3,
          padding: '1px 3px',
          overflow: 'hidden',
          textOverflow: 'ellipsis',
          whiteSpace: 'nowrap',
          textDecoration: e2.done ? 'line-through' : 'none'
        }
      }, e2.hora ? "".concat(e2.hora, " ") : '', e2.t);
    }), extraCount > 0 && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 8,
        fontWeight: 800,
        color: isSel ? 'rgba(255,255,255,0.85)' : F.muted,
        textAlign: 'center'
      }
    }, "+", extraCount));
  })))), mainView === 'mes' && /*#__PURE__*/React.createElement("div", {
