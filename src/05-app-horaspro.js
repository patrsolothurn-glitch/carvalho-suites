...
var EntryFormModal = React.memo(function EntryFormModal(_ref8) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      width: '100%',
      background: horas > 0 ? "linear-gradient(135deg,".concat(H.gold, ",").concat(H.goldL, ")") : '#ddd',
      border: 'none',
      borderRadius: 16,
      padding: '16px',
      color: horas > 0 ? '#fff' : '#aaa',
      fontSize: 16,
      fontWeight: 900,
      cursor: horas > 0 ? 'pointer' : 'default',
      boxShadow: horas > 0 ? "0 6px 20px rgba(184,150,46,0.35)" : 'none'
    }
  }, isEdit ? '✓ Guardar alterações' : '✓ Guardar registo', horas > 0 ? " \xB7 ".concat(horas, "h") : ''))));
});
function HorasProApp(_ref9) {
  ...
  }, "\u2713 ", editProj ? 'Guardar' : 'Adicionar'))), projList.filter(function (p) {
    ...
    onSave: function onSave(_ref10) {
      ...
      if (window.supabaseClient) {
        window.supabaseClient.from('horas_entries').update({
          ...
          hours: horas,
          address: morada || null
        }).eq('id', editEntry.id).then(function () {
          loadEntries();
        }).catch(function () {});
      }
      setEditEntry(null);
