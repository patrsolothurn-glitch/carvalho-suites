...
function hwNum(v) { return parseFloat(String(v).replace(',', '.')); }
...
var HauswartApp = function(props) {
        ...
        if (!res.error && res.data) {
          ...
          if (lw.length > 0 || lm.length > 0 || la.length > 0) {
            window.supabaseClient.from('hauswart_data').upsert(
              ...
              if (res2 && res2.error && res2.error.code !== 'PGRST205') {
                console.warn('loadHauswartData seed:', res2.error.message);
                window.mostrarErro('Hauswart', res2.error);
              }
            }).catch(function(e) {
              console.warn('loadHauswartData seed erro:', e);
    ...
    var doUpsert = function() {
      ...
      window.supabaseClient.from('hauswart_data').upsert(
        ...
        if (res && res.error && res.error.code !== 'PGRST205') {
          console.warn('hwSync:', res.error.message);
          window.mostrarErro('Hauswart', res.error);
          return;
        }
        lastUpdatedAtRef.current = nowIso;
      }).catch(function(e) {
        console.warn('hwSync erro:', e);
        window.mostrarErro('Hauswart', e);
      });
    };
    ...
    window.supabaseClient.from('hauswart_data').select('updated_at').eq('member_id', owner).single()
      .then(function(checkRes) {
        if (checkRes && !checkRes.error && checkRes.data && checkRes.data.updated_at) {
          ...
          if (serverTs > localTs) {
            console.warn('hwSync: conflito — servidor mais recente que o local, gravação cancelada');
            window.mostrarErro('Hauswart', { message: 'Estes dados foram alterados noutro aparelho. Não gravei para não apagar esse trabalho. Recarrega a app e repete.' });
            return;
          }
        }
        doUpsert();
  ...
  if (printMode) {
    var isReprint = printMode && typeof printMode === 'object';
    var pv = isReprint ? printMode : null;
    var pInvNum = isReprint ? (pv.invNum || (pv.invLabel || '').split(' ').pop()) : invNum;
    return React.createElement(HwPrintView, {
      works: isReprint ? pv.works : works,
      mats: isReprint ? pv.mats : mats,
      cfg: isReprint ? pv.cfg : cfg,
      total: isReprint ? pv.total : total,
      totalWork: isReprint ? pv.totalWork : totalWork,
      totalMats: isReprint ? pv.totalMats : totalMats,
      pauschale: isReprint ? pv.pauschale : pauschale,
      beschreibung: isReprint ? pv.beschreibung : beschreibung,
      referenz: isReprint ? pv.referenz : referenz,
      invLabel: isReprint ? pv.invLabel : invLabel,
      invNum: pInvNum,
      leistungszeitraum: isReprint ? pv.leistungszeitraum : leistungszeitraum,
      isReprint: isReprint,
      onBack: function() { setPrint(false); },
      onNew: isReprint ? null : function() {
...
function HwWorkTab(props) {
  ...
  var onSave = function() {
    var hours = hwNum(f.hours), rate = hwNum(f.rate);
    if (!f.description || !hours) { window.alert('Preenche a descrição e as horas (usa ponto ou vírgula, ex: 1.5 ou 1,5).'); return; }
    var entry = Object.assign({}, f, { hours: hours, rate: rate });
  ...
  if (view === 'form') {
    return React.createElement('div', null,
      ...
      f.hours && f.rate && React.createElement('div', { style: Object.assign({}, S.card, { borderLeft: '3px solid #22c55e', marginBottom: 16 }) },
        ...
        React.createElement('div', { style: { fontSize: 22, fontWeight: 700, color: '#22c55e' } }, hwChf(hwNum(f.hours||0) * hwNum(f.rate||0)))
      ),
...
function HwMatsTab(props) {
  ...
  var onSave = function() {
    var price = hwNum(f.price);
    if (!f.description || !price) { window.alert('Preenche a descrição e o preço (usa ponto ou vírgula, ex: 5.50 ou 5,50).'); return; }
    if (editing) updM(mats.map(function(m) { return m.id === f.id ? Object.assign({}, f, { price: price }) : m; }));
    else updM(mats.concat([Object.assign({}, f, { id: hwUid(), price: price })]));
    setView('list');
  };
...
function HwInvoiceTab(props) {
  ...
  return React.createElement('div', null,
    ...
    React.createElement('div', { style: Object.assign({}, S.card, { marginBottom: 14 }) },
      ...
      !cfg.invoiceDate && React.createElement('div', { style: { fontSize: 11, color: '#f87171', marginTop: -8, marginBottom: 10 } }, '⚠️ Obrigatório antes de imprimir'),
...
function HwArquivoTab(props) {
  var archive = props.archive, updA = props.updA, setPrint = props.setPrint, S = props.S;
  ...
  var markPaid = function(id) {
    updA(archive.map(function(a) {
      if (a.id !== id) return a;
      return a.paid
        ? Object.assign({}, a, { paid: false, datePaid: null })
        : Object.assign({}, a, { paid: true, datePaid: hwToday() });
    }));
  };
            ...
            return React.createElement('div', {
              ...
              React.createElement('div', { style: { textAlign: 'right', flexShrink: 0 } },
                ...
                React.createElement('div', { style: { display: 'flex', gap: 5, marginTop: 8, justifyContent: 'flex-end' } },
                  React.createElement('button', {
                    onClick: function() { setPrint(a); },
                    style: { background: '#1d4ed822', color: '#93c5fd', border: 'none', borderRadius: 6, padding: '5px 8px', fontSize: 12, cursor: 'pointer' }
                  }, '🖨️'),
                  React.createElement('button', {
                    onClick: function() { markPaid(a.id); },
                    style: { background: a.paid ? '#7c2d12' : '#14532d', color: a.paid ? '#fdba74' : '#4ade80', border: 'none', borderRadius: 6, padding: '5px 10px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }
                  }, a.paid ? '↺ Desmarcar' : '✓ Pago'),
                  React.createElement('button', {
...
function HwPrintView(props) {
  ...
  var onBack = props.onBack, onNew = props.onNew, isReprint = props.isReprint;
  ...
  React.useEffect(function() {
    ...
    var beforePrint = function() {
      document.querySelectorAll('.hw-sheet').forEach(function(pp) {
    ...
    var afterPrint = function() {
      document.querySelectorAll('.hw-sheet').forEach(function(pp) {
        if (pp.dataset.ps) { pp.style.transform = ''; pp.style.height = ''; delete pp.dataset.ps; }
      });
    };
  ...
  var dataFaltando = isReprint && !cfg.invoiceDate;
  var invoiceDateObj = cfg.invoiceDate ? new Date(cfg.invoiceDate + 'T12:00:00') : (isReprint ? null : new Date());
  var dateStr = invoiceDateObj ? invoiceDateObj.toLocaleDateString('de-CH') : '⚠️ data não registada';
  var faelligStr = invoiceDateObj ? hwPlus30Days(invoiceDateObj).toLocaleDateString('de-CH') : '—';
  ...
  var renderSheet = function(isKopie) {
    return React.createElement('div', {
      key: isKopie ? 'kopie' : 'orig',
      id: isKopie ? undefined : 'print-page',
      className: 'hw-sheet page' + (isKopie ? ' is-kopie' : '')
    },
      React.createElement('div', { className: 'body-flex' },
        ...
        React.createElement('div', { className: 'top' },
          ...
          React.createElement('div', { className: 'doctype' },
            ...
            React.createElement('div', { className: 'place' }, dataFaltando ? dateStr : 'Selzach, ' + dateStr),
            isKopie && React.createElement('div', { className: 'kopie-mark' }, 'Kopie')
          )
        ),
  ...
  return React.createElement('div', { className: 'hw-print-outer', style: { background: '#94a3b8' } },
    ...
    React.createElement('div', { className: 'no-print', style: { background: '#1e293b', padding: '10px 16px', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 } },
      ...
      React.createElement('button', { onClick: function() { if (dataFaltando) { window.alert('Esta fatura não tem data registada — não é possível imprimir sem isso.'); return; } window.print(); }, disabled: dataFaltando, style: { background: dataFaltando ? '#334155' : '#1d4ed8', color: dataFaltando ? '#64748b' : 'white', border: 'none', borderRadius: 8, padding: '8px 22px', cursor: dataFaltando ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 700 } }, '🖨️ Drucken / PDF'),
      React.createElement('button', {
        onClick: function() { setKopie(!kopie); },
      ...
      React.createElement('button', {
        onClick: function() {
          ...
          var html = '<!DOCTYPE html><html><head>'
            ...
            + '<div><img src="' + qrUrl + '" class="qrimg" alt="QR" onerror="var d=document.createElement(\'div\');d.className=\'qrimg qrerr\';d.style.cssText=\'background:#7f1d1d;border:1px solid #ef4444;color:#fff;font-weight:700;font-size:7px;line-height:1.3;padding:4px;box-sizing:border-box;display:flex;align-items:center;justify-content:center;text-align:center;\';d.textContent=\'⚠️ QR não carregou — NÃO IMPRIMIR. Verifica a ligação e recarrega.\';this.parentNode.replaceChild(d,this);"/>'
            + '<div class="row"><div class="col"><div class="lbl">Währung</div><div class="big">CHF</div></div><div class="col"><div class="lbl">Betrag</div><div class="big">' + total.toFixed(2) + '</div></div></div>'
            + '</div>'
      ...
      typeof onNew === 'function' && React.createElement('button', { onClick: onNew, style: { background: '#14532d', color: '#4ade80', border: 'none', borderRadius: 8, padding: '8px 14px', cursor: 'pointer', fontSize: 13 } }, '✓ Abgerechnet — Neue Rechnung')
    ),
    ...
    React.createElement('style', null,
      '.hw-sheet.page, .hw-sheet.page *{box-sizing:border-box;}' +
      '.hw-sheet.page{width:210mm;min-height:297mm;padding:18mm 18mm 14mm;display:flex;flex-direction:column;background:#fff;font-family:Helvetica,Arial,"Helvetica Neue",sans-serif;color:#14181d;box-shadow:0 8px 40px rgba(0,0,0,.3);}' +
      '.hw-sheet .body-flex{flex:1;}' +
      '.hw-sheet .top{display:flex;justify-content:space-between;align-items:flex-start;}' +
      '.hw-sheet .sender{font-size:9.5pt;line-height:1.5;}' +
      '.hw-sheet .sender .name{font-weight:700;font-size:10.5pt;}' +
      '.hw-sheet .sender .line{color:#6b7280;}' +
      '.hw-sheet .doctype{text-align:right;}' +
      '.hw-sheet .doctype h1{margin:0;font-size:17pt;font-weight:600;letter-spacing:.22em;text-transform:uppercase;}' +
      '.hw-sheet .doctype .nr{margin-top:4px;font-size:10pt;font-weight:600;color:#1d4ed8;font-variant-numeric:tabular-nums;}' +
      '.hw-sheet .doctype .place{margin-top:2px;font-size:9pt;color:#6b7280;}' +
      '.hw-sheet .rule{height:1px;background:#14181d;margin-top:12px;}' +
      '.hw-sheet .band{display:flex;justify-content:space-between;margin-top:18px;}' +
      '.hw-sheet .eyebrow{font-size:7.5pt;letter-spacing:.14em;text-transform:uppercase;color:#6b7280;margin-bottom:5px;}' +
      '.hw-sheet .recipient{font-size:10pt;line-height:1.5;}' +
      '.hw-sheet .recipient .org{font-weight:700;}' +
      '.hw-sheet .meta{width:76mm;font-size:9pt;border-top:1px solid #dfe3e8;margin:0;}' +
      '.hw-sheet .meta div{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #dfe3e8;}' +
      '.hw-sheet .meta dt{color:#6b7280;}' +
      '.hw-sheet .meta dd{margin:0;text-align:right;font-weight:600;font-variant-numeric:tabular-nums;}' +
      '.hw-sheet .subject{margin-top:18px;font-size:10pt;line-height:1.55;}' +
      '.hw-sheet table{width:100%;border-collapse:collapse;margin-top:14px;font-size:9.5pt;}' +
      '.hw-sheet thead th{font-size:7.5pt;letter-spacing:.12em;text-transform:uppercase;color:#6b7280;font-weight:600;text-align:left;padding:0 0 5px;border-bottom:1px solid #14181d;}' +
      '.hw-sheet thead th.num{text-align:right;}' +
      '.hw-sheet tr.section td{padding:14px 0 5px;font-size:7.5pt;letter-spacing:.14em;text-transform:uppercase;font-weight:700;border-bottom:1px solid #14181d;}' +
      '.hw-sheet tbody td{padding:8px 0;border-bottom:1px solid #dfe3e8;vertical-align:top;}' +
      '.hw-sheet tbody td.num{text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums;}' +
      '.hw-sheet .pos-title{font-weight:600;}' +
      '.hw-sheet .pos-sub{color:#6b7280;font-size:8pt;margin-top:2px;}' +
      '.hw-sheet tr.subtotal td{padding:6px 0;border-bottom:0;color:#6b7280;font-size:8.5pt;}' +
      '.hw-sheet tr.subtotal td.num{color:#14181d;font-weight:600;}' +
      '.hw-sheet tr{break-inside:avoid;page-break-inside:avoid;}' +
      '.hw-sheet .totals{margin-top:14px;display:flex;justify-content:flex-end;}' +
      '.hw-sheet .totals .box{width:76mm;}' +
      '.hw-sheet .totals .row{display:flex;justify-content:space-between;padding:4px 0;font-size:9pt;color:#6b7280;}' +
      '.hw-sheet .totals .row span:last-child{color:#14181d;font-variant-numeric:tabular-nums;}' +
      '.hw-sheet .totals .grand{display:flex;justify-content:space-between;align-items:baseline;margin-top:5px;padding-top:8px;border-top:2px solid #14181d;}' +
      '.hw-sheet .totals .grand .lbl{font-size:9pt;font-weight:700;letter-spacing:.08em;text-transform:uppercase;}' +
      '.hw-sheet .totals .grand .val{font-size:15pt;font-weight:700;font-variant-numeric:tabular-nums;}' +
      '.hw-sheet .totals .note{margin-top:5px;font-size:8pt;color:#6b7280;text-align:right;}' +
      '.hw-sheet .pay{margin-top:20px;padding-top:12px;border-top:1px solid #dfe3e8;display:flex;justify-content:space-between;font-size:9pt;line-height:1.55;}' +
      '.hw-sheet .pay .iban{font-family:"Courier New",monospace;font-size:9.5pt;letter-spacing:.04em;}' +
      '.hw-sheet .pay .term{text-align:right;}' +
      '.hw-sheet .pay .term .ref{font-weight:700;margin-top:2px;}' +
      '.hw-sheet footer{margin-top:auto;padding-top:12px;border-top:1px solid #dfe3e8;display:flex;justify-content:space-between;font-size:8pt;color:#6b7280;}' +
      '.hw-sheet .doctype .kopie-mark{margin-top:3px;font-size:9pt;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#6b7280;}' +
      '.hw-sheet.is-kopie{page-break-before:always;break-before:always;}'
    ),
    React.createElement('style', null,
      '@media print {' +
      '  @page { size: A4 portrait; margin: 0; }' +
      ...
      '  .hw-sheet.page { box-shadow: none !important; } ' +
...
function HwZahlteil(props) {
  ...
  var _useStateQrFail = React.useState(false);
  var qrFailed = _useStateQrFail[0], setQrFailed = _useStateQrFail[1];
  ...
  var zahl = React.createElement('div', { style: { flex: 1, padding: '5mm 5mm 5mm 10mm', display: 'flex', flexDirection: 'column', gap: '3mm', boxSizing: 'border-box' } },
    ...
    React.createElement('div', { style: { display: 'flex', gap: '5mm', alignItems: 'flex-start' } },
      ...
      React.createElement('div', null,
        qrFailed
          ? React.createElement('div', {
              style: { width: 133, height: 133, boxSizing: 'border-box', background: '#7f1d1d', border: '1px solid #ef4444', color: '#fff', fontWeight: 700, fontSize: 9, lineHeight: 1.3, padding: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }
            }, '⚠️ QR não carregou — NÃO IMPRIMIR. Verifica a ligação e recarrega.')
