...
var HauswartApp = function(props) {
    ...
    window.supabaseClient.from('hauswart_data').select('*').eq('member_id', owner).single()
      .then(function(res) {
        if (!res.error && res.data) {
          ...
          if (d.cfg   && Object.keys(d.cfg).length) { var m = Object.assign({}, HW_DEFAULTS, d.cfg); setCfgRaw(m); hwSave('cfg', m); }
  ...
  var hwSync = function(w, m, c, a) {
    ...
    window.supabaseClient.from('hauswart_data').upsert(
      ...
      if (res && res.error && res.error.code !== 'PGRST205') console.warn('Hauswart sync:', res.error.message);
    });
  };
...
// ── RECHNUNG — novo render impresso (A4, 1 página) ──
function fmtCHF(n) {
