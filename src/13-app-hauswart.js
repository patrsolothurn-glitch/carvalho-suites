      ...
      React.createElement('button', {
        onClick: function() {
          ...
          var html = '<!DOCTYPE html><html><head><title>Zahlteil ' + referenz + '</title>'
            ...
            + '</style>'
            + '<script>'
            + 'function scaleZahlteil() {'
            + '  var w = window.innerWidth - 16;'
            + '  var mmPx = 210 * 96 / 25.4;'
            + '  var s = w < mmPx ? w / mmPx : 1;'
            + '  document.documentElement.style.setProperty("--z-scale", s);'
            + '  var wrap = document.querySelector(".zahlteil-wrap");'
            + '  if (wrap) wrap.style.height = (wrap.scrollHeight * s) + "px";'
            + '}'
            + 'window.addEventListener("load", scaleZahlteil);'
            + 'window.addEventListener("resize", scaleZahlteil);'
            + '</script>'
            + '</head><body>'
            + '<div class="no-print">'
