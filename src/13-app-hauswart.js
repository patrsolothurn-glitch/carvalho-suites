      ...
      React.createElement('button', {
        onClick: function() {
          ...
          var html = '<!DOCTYPE html><html><head><title>Zahlteil ' + referenz + '</title>'
            ...
            + '.page { position: relative; width: 210mm; height: 297mm; margin: 16px auto; background: white; border: 1px solid #e0e0e0; overflow: hidden; }'
            + '.zahlteil-wrap { position: absolute; bottom: 0; left: 0; right: 0; height: 105mm; background: white; }'
            + '.scissors { display: flex; align-items: center; gap: 6px; padding: 1.5mm 2mm; }'
            + '.scissors span { font-size: 11px; color: #444; }'
            + '.scissors hr { flex: 1; border: none; border-top: 1px dashed #777; }'
            + '.slip { display: flex; border: 0.5pt solid #333; height: 100mm; }'
            ...
            + '.lbl { font-size: 6pt; font-weight: 700; text-transform: uppercase; letter-spacing: 0.3px; margin-bottom: 1mm; color: #000; }'
            + '.val { font-size: 8pt; line-height: 1.45; color: #000; }'
            + '.big { font-size: 10pt; font-weight: 700; color: #000; }'
