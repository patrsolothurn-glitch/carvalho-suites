...
@media print{\
  ...
  .wp-top,.wp-bar,.wp-kpis,.wp-cols,.wp-scroll,.wp-alarm,.wp-legend,.wp-abs,.wp-ov,.wp-week,.wp-noprint{display:none!important}\
  #wp-printArea{display:block!important}\
  .wp-ph{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #000;padding-bottom:5px;margin-bottom:9px}\
  .wp-ph .wp-lg{width:14px;height:14px;background:#12B886;display:inline-block;vertical-align:-2px;margin-right:6px}\
  .wp-pt2{width:100%;border-collapse:collapse;table-layout:fixed}\
  ...
  .wp-pd{font-size:9pt;font-weight:700;margin:8px 0 2px;break-after:avoid;page-break-after:avoid}\
  .wp-pl{display:flex;gap:7px;font-size:8.5pt;padding:2px 0;border-bottom:.5pt solid #ccc;break-inside:avoid}\
  .wp-pl .wp-b{width:12px;flex:none}.wp-pl .wp-t{width:70px;flex:none}.wp-pl .wp-n{width:58px;flex:none;text-align:right}\
  .wp-sum{display:flex;gap:16px;font-size:8.5pt;margin-top:6px;flex-wrap:wrap}\
  .wp-sig{display:flex;gap:26px;margin-top:20px;font-size:8pt;break-inside:avoid}\
  .wp-sig div{flex:1;border-top:.8pt solid #000;padding-top:3px}\
  .wp-pbreak{break-before:page;page-break-before:always}\
  .wp-pfoot{margin-top:6px;font-size:7pt;color:#666;text-align:right}\
}';

// ── Peças pequenas ao nível do módulo ─────────────────────────────
function wpStIcon(s) {
  var cor = s === 'erledigt' ? 'var(--ok)' : s === 'laeuft' ? 'var(--run)' : 'var(--ink3)';
  var ic = s === 'erledigt' ? '✓' : s === 'laeuft' ? '◐' : '○';
    ...
    React.createElement('div', { className: 'wp-sheet', onClick: function(e) { e.stopPropagation(); } },
      ...
      p.id && React.createElement('div', null,
        ...
        React.createElement('div', { className: 'wp-prios' }, [['offen', 'offen'], ['laeuft', 'läuft'], ['erledigt', 'erledigt']].map(function(o) {
          return React.createElement('button', { key: o[0], className: 'wp-pb', onClick: function() { p.onChange('status', o[0]); }, style: { borderColor: d.status === o[0] ? 'var(--ink)' : 'var(--line)' } }, o[1]);
        }))
      ),
...
function wpKopfZeile(titulo, seite, cur, seite2) {
  ...
  return React.createElement('div', { className: 'wp-ph' },
    ...
    React.createElement('div', { style: { textAlign: 'right', fontSize: '8.5pt' } }, seite2 || '')
  );
}
function WpPrintPlan(p) {
  ...
  return React.createElement(React.Fragment, null,
    wpKopfZeile('Einsatzplan', p.seite, p.cur, p.seite),
      ...
      React.createElement('tbody', null, linhas.map(function(nm) {
        ...
        var cols = W.map(function(k) {
          ...
          return React.createElement('td', { key: k },
            ...
            L.map(function(a) {
              return React.createElement('div', { key: a.id, className: 'wp-pj' }, React.createElement('b', null, a.von + '–' + a.bis), (a.status === 'erledigt' ? '☒ ' : a.status === 'laeuft' ? '◐ ' : '☐ ') + a.titel + (a.auftrag_nr ? ' ' : ''), a.auftrag_nr && React.createElement('b', null, a.auftrag_nr));
            })
          );
        });
...
function WpPrintBericht(p) {
  ...
  W.forEach(function(a) { plan += wpDur(a); if (a.status === 'erledigt') ist += wpDur(a); });
  return React.createElement(React.Fragment, null,
    wpKopfZeile('Wochenbericht', p.seite, p.cur, p.seite),
    ...
    days.map(function(k, i) {
      ...
      return React.createElement('div', { key: k },
        ...
        L.map(function(a) {
          return React.createElement('div', { key: a.id, className: 'wp-pl' }, React.createElement('span', { className: 'wp-b' }, a.status === 'erledigt' ? '☒' : a.status === 'laeuft' ? '◐' : '☐'), React.createElement('span', { className: 'wp-t' }, a.von + '–' + a.bis), React.createElement('span', { style: { flex: 1 } }, a.titel + ' · ' + a.arbeit + (a.auftrag_nr ? ' · ' + a.auftrag_nr : '')), React.createElement('span', { className: 'wp-n' }, a.wer || ''));
        }),
...
function WpPrintListe(p) {
  ...
  var dr = wpPoolL(p.tasks, p.who, true);
  return React.createElement(React.Fragment, null,
    wpKopfZeile(nurTag ? 'Tagesliste' : 'Wochenliste', null, p.cur, null),
    tage.map(function(k) {
