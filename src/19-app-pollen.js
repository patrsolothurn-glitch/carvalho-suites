...
// também partilhado. pollen_perfis.medicamentos (jsonb, lista de
// {id, nome, toma}) e pollen_diario.medicamentos_tomados (jsonb, lista
// de {id, nome}) — a coluna antiga pollen_diario.medicamento (texto)
// fica só para leitura de dias antigos, nunca mais escrita por dias
// novos (ver renderDiario/guardarDiario). pollen_estacoes/pollen_medicoes
// são só de leitura aqui (alimentadas pelo workflow
// .github/workflows/pollen-fetch.yml). Nunca mistura com tabelas de
// outra app.
// ══════════════════════════════════════════════════════════════════
...
// de hwUid em src/13-app-hauswart.js) ──────────────────────────────
function polUid() { return 'm_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 7); }
...
var POL_CSS = '' +
  ...
  '.pol-med-row{display:flex;align-items:center;gap:10px;padding:12px 10px;border-radius:12px;border:1px solid var(--pol-borda);background:var(--pol-cartao);cursor:pointer}' +
  '.pol-med-check{flex:none;width:28px;height:28px;border-radius:8px;border:2px solid var(--pol-borda);display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;color:var(--pol-verde-texto)}' +
  '.pol-med-check.pol-med-check-ativo{background:var(--pol-verde);border-color:var(--pol-verde)}';
...
function PollenApp(props) {
  ...
  var _s39 = React.useState([]); var pfMedicamentos = _s39[0], setPfMedicamentos = _s39[1];
  var _s40 = React.useState(false); var pfMedFormAberto = _s40[0], setPfMedFormAberto = _s40[1];
  var _s41 = React.useState(''); var pfMedNome = _s41[0], setPfMedNome = _s41[1];
  var _s42 = React.useState(''); var pfMedToma = _s42[0], setPfMedToma = _s42[1];
  var _s43 = React.useState(null); var pfMedEditandoId = _s43[0], setPfMedEditandoId = _s43[1];
  var _s44 = React.useState(null); var confirmApagarMedPerfil = _s44[0], setConfirmApagarMedPerfil = _s44[1];
  var _s45 = React.useState([]); var fMedsChecked = _s45[0], setFMedsChecked = _s45[1];
  var _s46 = React.useState([]); var fMedsOutros = _s46[0], setFMedsOutros = _s46[1];
  ...
  function abrirNovoPerfil() {
    ...
    setPfMedicamentos([]); setPfMedFormAberto(false); setPfMedEditandoId(null); setPfMedNome(''); setPfMedToma('');
  ...
  function abrirEditarPerfil(p) {
    ...
    setPfMedicamentos(p.medicamentos || []); setPfMedFormAberto(false); setPfMedEditandoId(null); setPfMedNome(''); setPfMedToma('');
    setPfPesquisaCidade(''); setPfResultadosPesquisa([]);
    setPerfilEditorAberto(true);
  }
  ...
  function iniciarNovoMedPerfil() { setPfMedEditandoId(null); setPfMedNome(''); setPfMedToma(''); setPfMedFormAberto(true); }
  function iniciarEditarMedPerfil(m) { setPfMedEditandoId(m.id); setPfMedNome(m.nome); setPfMedToma(m.toma || ''); setPfMedFormAberto(true); }
  function guardarMedPerfil() {
    if (!pfMedNome.trim()) return;
    setPfMedicamentos(function (atual) {
      if (pfMedEditandoId) {
        return atual.map(function (m) { return m.id === pfMedEditandoId ? { id: m.id, nome: pfMedNome.trim(), toma: pfMedToma.trim() || null } : m; });
      }
      return atual.concat([{ id: polUid(), nome: pfMedNome.trim(), toma: pfMedToma.trim() || null }]);
    });
    setPfMedFormAberto(false);
  }
  function apagarMedPerfil(id) {
    setPfMedicamentos(function (atual) { return atual.filter(function (m) { return m.id !== id; }); });
    setConfirmApagarMedPerfil(null);
  }
  ...
  function guardarPerfil() {
    ...
    var payload = { nome: pfNome.trim(), alergias: pfAlergias, kanton: pfLocal.kanton || null, cidade: pfLocal.cidade, lat: pfLocal.lat, lon: pfLocal.lon, notificar: pfNotificar, avisar_ids: pfAvisarIds, medicamentos: pfMedicamentos };
  ...
  function iniciarNovoRegisto() {
    setDiarioEditandoId(null); setFData(polHojeIso()); setFSintomas(0); setFNota('');
    setFMedsChecked([]); setFMedsOutros([]);
  }
  function editarRegisto(r) {
    setDiarioEditandoId(r.id); setFData(r.data); setFSintomas(r.sintomas); setFNota(r.nota || '');
    var tomados = r.medicamentos_tomados || [];
    var idsPerfil = (perfilAtivo && perfilAtivo.medicamentos || []).map(function (m) { return m.id; });
    setFMedsChecked(tomados.map(function (t) { return t.id; }));
    setFMedsOutros(tomados.filter(function (t) { return idsPerfil.indexOf(t.id) === -1; }));
  }
  function toggleMedForm(id) {
    setFMedsChecked(function (atual) {
      return atual.indexOf(id) !== -1 ? atual.filter(function (i) { return i !== id; }) : atual.concat([id]);
    });
  }
  function adicionarMedOutro() {
    var novoId = polUid();
    setFMedsOutros(function (atual) { return atual.concat([{ id: novoId, nome: '' }]); });
    setFMedsChecked(function (atual) { return atual.concat([novoId]); });
  }
  function editarNomeMedOutro(id, nome) {
    setFMedsOutros(function (atual) { return atual.map(function (o) { return o.id === id ? { id: o.id, nome: nome } : o; }); });
  }
  function removerMedOutro(id) {
    setFMedsOutros(function (atual) { return atual.filter(function (o) { return o.id !== id; }); });
    setFMedsChecked(function (atual) { return atual.filter(function (i) { return i !== id; }); });
  }
  function guardarDiario() {
    if (!perfilAtivo) return;
    setDiarioSaving(true);
    var medsDoPerfil = (perfilAtivo.medicamentos || []).filter(function (m) { return fMedsChecked.indexOf(m.id) !== -1; }).map(function (m) { return { id: m.id, nome: m.nome }; });
    var medsOutros = fMedsOutros.filter(function (o) { return fMedsChecked.indexOf(o.id) !== -1 && o.nome && o.nome.trim(); }).map(function (o) { return { id: o.id, nome: o.nome.trim() }; });
    var medicamentosTomados = medsDoPerfil.concat(medsOutros);
    db.from('pollen_diario').upsert({ perfil_id: perfilAtivo.id, data: fData, sintomas: fSintomas, medicamentos_tomados: medicamentosTomados, nota: fNota || null }, { onConflict: 'perfil_id,data' }).select().then(function (res) {
  ...
  function renderDiario() {
    ...
    return React.createElement('div', { style: { padding: 16, display: 'flex', flexDirection: 'column', gap: 12 } },
      React.createElement(PolCard, null,
        ...
        React.createElement('p', { style: { fontSize: 11, fontWeight: 700, color: 'var(--pol-texto2)', textTransform: 'uppercase', margin: '2px 0 6px' } }, 'Medicamentos'),
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 } },
          (perfilAtivo.medicamentos || []).map(function (m) {
            var checked = fMedsChecked.indexOf(m.id) !== -1;
            return React.createElement('div', { key: m.id, className: 'pol-med-row', onClick: function () { toggleMedForm(m.id); } },
              React.createElement('div', { className: 'pol-med-check' + (checked ? ' pol-med-check-ativo' : '') }, checked ? '✓' : ''),
              React.createElement('div', { style: { flex: 1, minWidth: 0 } },
                React.createElement('div', { style: { fontWeight: 700, fontSize: 14 } }, m.nome),
                m.toma && React.createElement('div', { style: { fontSize: 11, color: 'var(--pol-texto2)', marginTop: 2 } }, m.toma)
              )
            );
          }),
          fMedsOutros.map(function (o) {
            var checked = fMedsChecked.indexOf(o.id) !== -1;
            return React.createElement('div', { key: o.id, className: 'pol-med-row', onClick: function () { toggleMedForm(o.id); } },
              React.createElement('div', { className: 'pol-med-check' + (checked ? ' pol-med-check-ativo' : '') }, checked ? '✓' : ''),
              React.createElement('input', { type: 'text', className: 'pol-input', autoComplete: 'off', placeholder: 'Nome do medicamento', value: o.nome, onClick: function (e) { e.stopPropagation(); }, onChange: function (e) { editarNomeMedOutro(o.id, e.target.value); }, style: { flex: 1, padding: '6px 8px', fontSize: 13 } }),
              React.createElement('button', { onClick: function (e) { e.stopPropagation(); removerMedOutro(o.id); }, style: { background: 'none', border: 'none', fontSize: 15, cursor: 'pointer', flex: 'none' } }, '✕')
            );
          }),
          !(perfilAtivo.medicamentos || []).length && !fMedsOutros.length && React.createElement('div', { style: { padding: '4px 0 8px' } },
            React.createElement('p', { style: { fontSize: 12.5, color: 'var(--pol-texto2)', marginBottom: 8 } }, 'Sem medicamentos — acrescenta no perfil'),
            React.createElement('button', { className: 'pol-btn', onClick: function () { abrirEditarPerfil(perfilAtivo); setTab('definicoes'); } }, 'Abrir perfil')
          ),
          React.createElement('button', { className: 'pol-btn', onClick: adicionarMedOutro }, '+ Outro')
        ),
      ...
      React.createElement(PolCard, { style: { padding: 0 } },
        diario.length
          ? diario.map(function (r) {
              var medsTomados = r.medicamentos_tomados || [];
              var medsTexto = medsTomados.length
                ? medsTomados.map(function (m) { return '✓ ' + m.nome; }).join(', ')
                : (r.medicamento || null);
              return React.createElement('div', { key: r.id, style: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderTop: '1px solid var(--pol-borda)' } },
                React.createElement('div', { style: { flex: 1 } },
                  ...
                  (medsTexto || r.nota) && React.createElement('div', { style: { fontSize: 11, color: 'var(--pol-texto2)' } }, [medsTexto, r.nota].filter(Boolean).join(' · '))
                ),
  ...
  function renderPerfilEditor() {
    return React.createElement(PolCard, { style: { marginBottom: 12 } },
      ...
      React.createElement('p', { style: { fontSize: 11, fontWeight: 700, color: 'var(--pol-texto2)', textTransform: 'uppercase', marginBottom: 6 } }, 'Medicamentos'),
      React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 8 } },
        pfMedicamentos.map(function (m) {
          return React.createElement('div', { key: m.id, style: { display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: 'var(--pol-fundo)', borderRadius: 10 } },
            React.createElement('div', { style: { flex: 1, minWidth: 0 } },
              React.createElement('div', { style: { fontWeight: 700, fontSize: 13 } }, m.nome),
              m.toma && React.createElement('div', { style: { fontSize: 11, color: 'var(--pol-texto2)' } }, m.toma)
            ),
            React.createElement('button', { onClick: function () { iniciarEditarMedPerfil(m); }, style: { background: 'none', border: 'none', fontSize: 14, cursor: 'pointer' } }, '✏️'),
            React.createElement('button', { onClick: function () { setConfirmApagarMedPerfil(m.id); }, style: { background: 'none', border: 'none', fontSize: 14, cursor: 'pointer' } }, '🗑️')
          );
        }),
        !pfMedicamentos.length && React.createElement('p', { style: { fontSize: 12.5, color: 'var(--pol-texto2)' } }, 'Ainda sem medicamentos.')
      ),
      pfMedFormAberto
        ? React.createElement(PolCard, { style: { marginBottom: 12 } },
            React.createElement('input', { type: 'text', className: 'pol-input', autoComplete: 'off', placeholder: 'Nome do medicamento', value: pfMedNome, onChange: function (e) { setPfMedNome(e.target.value); }, style: { marginBottom: 8 } }),
            React.createElement('input', { type: 'text', className: 'pol-input', autoComplete: 'off', placeholder: 'Toma (opcional, ex.: 1x por dia · em urgência 2x)', value: pfMedToma, onChange: function (e) { setPfMedToma(e.target.value); }, style: { marginBottom: 8 } }),
            React.createElement('div', { style: { display: 'flex', gap: 8 } },
              React.createElement('button', { className: 'pol-btn', style: { flex: 1 }, onClick: function () { setPfMedFormAberto(false); } }, 'Cancelar'),
              React.createElement('button', { className: 'pol-btn pol-btn-ativo', style: { flex: 1 }, onClick: guardarMedPerfil }, pfMedEditandoId ? 'Guardar alterações' : '✓ Adicionar')
            )
          )
        : React.createElement('button', { className: 'pol-btn', style: { marginBottom: 12 }, onClick: iniciarNovoMedPerfil }, '+ Novo medicamento'),
  ...
  var modalConfirmApagarMedPerfil = confirmApagarMedPerfil && React.createElement('div', {
    style: { position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 },
    onClick: function (e) { if (e.target === e.currentTarget) setConfirmApagarMedPerfil(null); }
  },
    React.createElement(PolCard, { style: { maxWidth: 340, width: '100%' } },
      React.createElement('p', { style: { fontWeight: 700, marginBottom: 16 } }, 'Apagar este medicamento?'),
      React.createElement('div', { style: { display: 'flex', gap: 10 } },
        React.createElement('button', { className: 'pol-btn', style: { flex: 1 }, onClick: function () { setConfirmApagarMedPerfil(null); } }, 'Cancelar'),
        React.createElement('button', { className: 'pol-btn', style: { flex: 1, background: '#DC2626', color: '#fff', borderColor: '#DC2626' }, onClick: function () { apagarMedPerfil(confirmApagarMedPerfil); } }, 'Apagar')
      )
    )
  );
