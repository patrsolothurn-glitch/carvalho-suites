// ══════════════════════════════════════════════════════════════════
// APP PÓLEN — Previsão e diário de alergias ao pólen (admin only, só
// Patricio). Só lê/escreve em pollen_perfis, pollen_diario e
// pollen_termos. pollen_estacoes/pollen_medicoes são só de leitura
...
function PollenApp(props) {
  ...
  function carregar() {
    ...
    setLoading(true);
    Promise.all([
      db.from('pollen_perfis').select('*').eq('profile_id', profile.id).order('created_at', { ascending: true }),
      db.from('pollen_termos').select('*').eq('profile_id', profile.id).order('created_at', { ascending: true }),
      db.from('pollen_estacoes').select('*')
    ]).then(function (res) {
      var perfisRes = res[0], termosRes = res[1], estacoesRes = res[2];
  ...
  function guardarPerfil() {
    ...
    var payload = { nome: pfNome.trim(), alergias: pfAlergias, kanton: pfLocal.kanton || null, cidade: pfLocal.cidade, lat: pfLocal.lat, lon: pfLocal.lon, notificar: pfNotificar };
  ...
  function guardarTermo() {
    ...
    query.then(function (res) {
      ...
      db.from('pollen_termos').select('*').eq('profile_id', profile.id).order('created_at', { ascending: true }).then(function (res2) {
