    ...
    form !== null && RpOverlay({
      ...
      children: React.createElement('div', null,
        ...
        kwTages.length > 0 && React.createElement('div', { style: { marginBottom: 14 } },
          React.createElement('div', { style: { fontSize: 11, fontWeight: 700, color: '#F59E0B', letterSpacing: '0.5px', marginBottom: 8 } },
            '📄 ' + (props.lang === 'PT' ? 'Relatórios desta KW' : 'Tagesrapporte dieser KW') + ' (' + kwTages.length + ')'
          ),
          kwTages.map(function(tr) {
            ...
            return React.createElement('div', {
              key: tr.id,
              style: { background: '#111827', border: '1px solid #1E293B', borderRadius: 10, marginBottom: 6, overflow: 'hidden' }
            },
              React.createElement('div', {
                ...
                React.createElement('div', { style: { flex: 1, minWidth: 0 } },
                  ...
                  React.createElement('div', { style: { fontSize: 11, color: '#64748B' } }, (tr.h_start || '—') + '–' + (tr.h_end || '—') + ' · ' + h.toFixed(1) + 'h' + (tr.km > 0 ? ' · ' + tr.km + ' km' : ''))
                ),
                React.createElement('span', { style: { color: '#475569', fontSize: 14, marginLeft: 4 } }, isOpen ? '▲' : '▼')
              ),
