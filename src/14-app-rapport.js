...
function RpTagesView(props) {
  ...
  // Wochenplan (wplan_tasks) prefill
  var _wpOpts   = React.useState([]);               var wpOpts = _wpOpts[0], setWpOpts = _wpOpts[1];
  var _wpTask   = React.useState(null);             var wpTask = _wpTask[0], setWpTask = _wpTask[1];
  ...
  // ── Wochenplan → Tagesrapport (leitura apenas, nunca escreve em wplan_tasks)
  function wpApply(t) {
    if (!t) return;
    setWpTask(t);
    setWpOpts([]);
    if (t.kunde) setFKunde(t.kunde);
    if (t.titel) setFOrt(t.titel);
    if (t.auftrag_nr) setFAuftrag(t.auftrag_nr);
    if (t.arbeit) setFBeschrieb(t.arbeit);
    if (t.wer) setFTeam(t.wer);
    if (t.von) setFHS(t.von);
    if (t.bis) setFHE(t.bis);
  }
  function wpClear() {
    setWpTask(null);
    setFKunde(''); setFOrt(''); setFAuftrag(''); setFBeschrieb(''); setFTeam('');
    setFHS('07:00'); setFHE('17:00');
  }
  function wpLoadFor(datum) {
    if (!db || !datum) return;
    setWpOpts([]); setWpTask(null);
    db.from('wplan_tasks').select('*').eq('datum', datum).order('von')
      .then(function(r) {
        if (r.error) return;
        var usados = {};
        (list || []).forEach(function(x) { if (x.wplan_task_id) usados[String(x.wplan_task_id)] = 1; });
        var livres = (r.data || []).filter(function(t) { return !usados[String(t.id)]; });
        if (livres.length === 1) wpApply(livres[0]);
        else if (livres.length > 1) setWpOpts(livres);
      })
      .catch(function() {});
  }
  function openNew() { resetForm(); setForm('new'); wpLoadFor(rpToday()); }
  ...
  function save() {
    ...
    var payload = {
      ...
      probleme: fProb.trim(),
      wplan_task_id: wpTask ? String(wpTask.id) : ((form && form !== 'new' && form.wplan_task_id) ? form.wplan_task_id : null)
    };
    var op = form === 'new'
  ...
  return React.createElement('div', { style: { padding: '12px 14px', paddingBottom: 80, maxWidth: 600, margin: '0 auto' } },
    ...
    form !== null && RpOverlay({
      ...
      children: React.createElement('div', null,
        ...
        // ── Wochenplan: aviso quando foi copiado
        wpTask && React.createElement('div', {
          style: { background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: 10, padding: '8px 10px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }
        },
          React.createElement('div', { style: { flex: 1, minWidth: 0 } },
            React.createElement('div', { style: { fontSize: 11, fontWeight: 700, color: '#10B981' } }, '📋 Aus Wochenplan übernommen'),
            React.createElement('div', { style: { fontSize: 11, color: '#94A3B8', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } },
              [wpTask.titel, wpTask.von && wpTask.bis ? wpTask.von + '–' + wpTask.bis : ''].filter(Boolean).join(' · '))
          ),
          RpBtn({ label: '↺ Leeren', sm: true, ghost: true, onClick: wpClear })
        ),

        // ── Wochenplan: escolher quando há mais do que um
        wpOpts.length > 1 && React.createElement('div', {
          style: { background: '#111827', border: '1px solid #1E293B', borderRadius: 10, padding: '8px 10px', marginBottom: 12 }
        },
          React.createElement('div', { style: { fontSize: 11, fontWeight: 700, color: '#F59E0B', marginBottom: 6 } }, '📋 ' + wpOpts.length + ' Einsätze im Wochenplan'),
          wpOpts.map(function(t) {
            return React.createElement('button', {
              key: String(t.id),
              onClick: function() { wpApply(t); },
              style: { display: 'block', width: '100%', textAlign: 'left', background: 'transparent', border: '1px solid #2A2A38', borderRadius: 8, padding: '6px 8px', marginBottom: 5, color: '#CBD5E1', fontSize: 12, cursor: 'pointer' }
            },
              React.createElement('div', { style: { fontWeight: 700 } }, t.titel || '—'),
              React.createElement('div', { style: { fontSize: 11, color: '#64748B', marginTop: 1 } },
                [t.von && t.bis ? t.von + '–' + t.bis : '', t.kunde, t.wer].filter(Boolean).join(' · '))
            );
          }),
          React.createElement('button', {
            onClick: function() { setWpOpts([]); },
            style: { background: 'none', border: 'none', color: '#64748B', fontSize: 11, cursor: 'pointer', padding: '2px 0' }
          }, 'Ohne Wochenplan ausfüllen')
        ),
        ...
        React.createElement('div', { style: { display: 'flex', gap: 10, marginBottom: 12 } },
          ...
          React.createElement('div', { style: { flex: 2 } }, RpLabel({ text: 'Datum' }), RpInput({ type: 'date', value: fDate, onChange: function(v) { setFDate(v); if (form === 'new') wpLoadFor(v); } }))
        ),
