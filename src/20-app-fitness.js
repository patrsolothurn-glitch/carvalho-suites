...
// Paleta fixa por posição da refeição no Plano (repete a partir da 7.ª).
var FI_CORES_REFEICAO = ['#1E8E3E', '#2563EB', '#B45309', '#7C3AED', '#DC2626', '#0891B2'];
function fiCorRefeicao(indice) { return FI_CORES_REFEICAO[indice % FI_CORES_REFEICAO.length]; }
var FI_CSS = '' +
  ...
  '.fi-refeicao-corpo{position:relative;padding-left:12px}' +
  '.fi-refeicao-barra{position:absolute;left:0;top:2px;bottom:2px;width:4px;border-radius:4px}' +
  '.fi-refeicao-num{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;color:#fff;font-size:12px;font-weight:900;flex:none}' +
  '.fi-anel-refeicao-wrap{display:flex;flex-direction:column;align-items:center;gap:2px;flex:none}' +
  '.fi-anel-refeicao{position:relative;width:44px;height:44px}' +
  '.fi-anel-refeicao-svg{width:44px;height:44px;display:block}' +
  '.fi-anel-refeicao-num{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:900}' +
  '.fi-anel-refeicao-pct{font-size:9.5px;color:var(--fi-texto2);font-weight:700}' +
  '.fi-ingredientes-toggle{margin-top:8px;font-size:12px;font-weight:700;color:var(--fi-texto2);cursor:pointer}' +
  '.fi-macro-bar{display:flex;height:5px;border-radius:3px;overflow:hidden;margin-top:6px}' +
  ...
  '.fi-progresso-barra-fundo{height:8px;border-radius:4px;background:var(--fi-borda);overflow:hidden}' +
  '.fi-progresso-barra-cheia{height:100%;background:var(--fi-verde);border-radius:4px}' +
  '.fi-grafico-scroll{overflow-x:auto;-webkit-overflow-scrolling:touch}' +
...
// Porções mostradas ao utilizador: arredonda ao 0.5 mais próximo, mínimo 0.5,
// sem casas decimais quando dá inteiro. As gramas exatas (kcal/macros) não usam isto.
function fiFmtUnidades(n) {
  if (n == null) return null;
  var arred = Math.max(0.5, Math.round(n * 2) / 2);
  return (arred % 1 === 0) ? String(arred) : arred.toFixed(1);
}
// Emoji do prato, por palavras-chave no nome da opção + nomes dos ingredientes
// (minúsculas, sem acentos). Sem correspondência → 🍽️.
function fiIconeOpcao(nome, nomesIngredientes) {
  function normalizar(s) {
    var t = (s || '').toLowerCase();
    return t.normalize ? t.normalize('NFD').replace(/[̀-ͯ]/g, '') : t;
  }
  var texto = normalizar(nome) + ' ' + (nomesIngredientes || []).map(normalizar).join(' ');
  var regras = [
    [/\b(frango|peru|carne)\b/, '🍗'],
    [/\b(vaca|vitela|porco)\b/, '🥩'],
    [/\b(peixe|atum|bacalhau|salmao)\b/, '🐟'],
    [/\b(camarao|marisco)\b/, '🦐'],
    [/\b(arroz|massa|batata|quinoa)\b/, '🍚'],
    [/\b(pao|sandes|tosta)\b/, '🥪'],
    [/\b(iogurte|leite|queijo|mousse|pudim)\b/, '🥛'],
    [/\bovo\b/, '🥚'],
    [/\b(fruta|banana|maca|laranja)\b/, '🍌'],
    [/\b(salada|legumes|sopa)\b/, '🥗'],
    [/\b(bolacha|cereais|aveia)\b/, '🍪']
  ];
  for (var i = 0; i < regras.length; i++) {
    if (regras[i][0].test(texto)) return regras[i][1];
  }
  return '🍽️';
}
...
// "DD-MM", sem ano — usado nas legendas do gráfico e no resumo do Progresso.
function fiFmtDataCurtaSemAno(dataISO) {
  if (!dataISO) return '—';
  var p = dataISO.split('-');
  return p[2] + '-' + p[1];
}
...
function FitnessApp(props) {
  ...
  var _s75 = React.useState({}); var opcoesAbertas = _s75[0], setOpcoesAbertas = _s75[1]; // { [opcaoId]: true } — lista de ingredientes aberta
  ...
  var _s87 = React.useState(null); var confirmApagarPreparo = _s87[0], setConfirmApagarPreparo = _s87[1]; // id da opção
  ...
  // Progresso (Fase 3)
  var _s76 = React.useState([]); var avaliacoes = _s76[0], setAvaliacoes = _s76[1]; // ordenadas por data ascendente
  var _s77 = React.useState(false); var avaliacoesCarregando = _s77[0], setAvaliacoesCarregando = _s77[1];
  var _s78 = React.useState(false); var avalFormAberto = _s78[0], setAvalFormAberto = _s78[1];
  var _s79 = React.useState(fiAvaliacaoVazia()); var avalForm = _s79[0], setAvalForm = _s79[1];
  var _s80 = React.useState(null); var avalEditandoId = _s80[0], setAvalEditandoId = _s80[1];
  var _s81 = React.useState(false); var avalSaving = _s81[0], setAvalSaving = _s81[1];
  var _s82 = React.useState(null); var confirmApagarAvaliacao = _s82[0], setConfirmApagarAvaliacao = _s82[1];
  var _s83 = React.useState(null); var avalConflito = _s83[0], setAvalConflito = _s83[1]; // avaliação já existente nessa data
  var _s84 = React.useState(null); var compararA = _s84[0], setCompararA = _s84[1];
  ...
  function carregarAvaliacoes() {
    ...
    db.from('fitness_avaliacoes').select('*').order('data', { ascending: true }).then(function (res) {
      setAvaliacoesCarregando(false);
      if (res.error) { console.error('[fitness] carregar avaliações:', res.error); window.mostrarErro('Fitness', res.error); return; }
      setAvaliacoes(res.data || []);
    }).catch(function (e) {
      setAvaliacoesCarregando(false);
      console.error('[fitness] carregar avaliações:', e);
      window.mostrarErro('Fitness', e);
    });
  }
  ...
  function apagarPreparo(id) {
    db.from('fitness_opcoes').update({ preparo: null }).eq('id', id).then(function (res) {
      if (res.error) { console.error('[fitness] apagar preparo:', res.error); window.mostrarErro('Fitness', res.error); return; }
      setConfirmApagarPreparo(null);
      setPreparoEditAberto(false);
      carregar();
  ...
  function abrirNovoItem(opcaoId) {
    setItemOpcaoAberta(opcaoId); setItemEditandoId(null); setItemForm({ alimento_id: alimentos[0] ? alimentos[0].id : '', gramas_base: '', ajustavel: true, nota: '' });
    setOpcoesAbertas(function (m) { var n = Object.assign({}, m); n[opcaoId] = true; return n; });
  }
  function abrirEditarItem(it) {
    setItemOpcaoAberta(it.opcao_id); setItemEditandoId(it.id); setItemForm({ alimento_id: it.alimento_id, gramas_base: String(it.gramas_base), ajustavel: it.ajustavel !== false, nota: it.nota || '' });
    setOpcoesAbertas(function (m) { var n = Object.assign({}, m); n[it.opcao_id] = true; return n; });
  }
  ...
  function renderOpcao(o, kcalRefeicao, irmas, idx) {
    ...
    var aberto = !!opcoesAbertas[o.id];
    var icone = fiIconeOpcao(o.nome, itensOpcao.map(function (it) { var al = alimentosPorId[it.alimento_id]; return al ? al.nome : ''; }));
    var kcalP = calc.totais.prot * 4, kcalH = calc.totais.hc * 4, kcalG = calc.totais.gord * 9;
    var totalMacroKcal = kcalP + kcalH + kcalG;
    return React.createElement(FiCard, { key: o.id, style: { marginBottom: 10 } },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
        React.createElement('div', { style: { flex: 1, cursor: opcaoEditandoId === o.id ? 'default' : 'pointer' }, onClick: opcaoEditandoId === o.id ? undefined : function () { abrirReceita(o); } },
          opcaoEditandoId === o.id
            ...
            : React.createElement('div', { style: { fontWeight: 800, fontSize: 14 } }, icone + ' ' + (o.favorito ? '★ ' : '') + o.nome),
          ...
          calc.itens.length > 0 && totalMacroKcal > 0 && React.createElement('div', { className: 'fi-macro-bar' },
            React.createElement('div', { style: { width: (kcalP / totalMacroKcal * 100) + '%', background: '#2563EB' } }),
            React.createElement('div', { style: { width: (kcalH / totalMacroKcal * 100) + '%', background: '#B45309' } }),
            React.createElement('div', { style: { width: (kcalG / totalMacroKcal * 100) + '%', background: '#DC2626' } })
          )
        ),
      ...
      React.createElement('div', {
        ...
        onClick: function () { setOpcoesAbertas(function (m) { var n = Object.assign({}, m); n[o.id] = !n[o.id]; return n; }); }
      }, calc.itens.length + ' ingrediente' + (calc.itens.length === 1 ? '' : 's') + ' ' + (aberto ? '▾' : '▸')),
      aberto && React.createElement('div', { style: { marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 } },
        calc.itens.map(function (it) {
          return React.createElement('div', { key: it.id, style: { padding: '4px 0', borderTop: '1px solid var(--fi-borda)' } },
            React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 } },
              React.createElement('span', null, it.nome + ' — ' + Math.round(it.gramas) + ' ' + it.medida + (it.unidades != null ? ' (' + fiFmtUnidades(it.unidades) + ' ' + (it.unidade_nome || 'unid.') + ')' : '') + (it.ajustavel ? '' : ' · fixo')),
              React.createElement('div', { style: { display: 'flex', gap: 6 } },
      ...
      itemOpcaoAberta === o.id
        ...
        : (aberto && React.createElement('button', { className: 'fi-btn', style: { marginTop: 8, width: '100%' }, onClick: function () { abrirNovoItem(o.id); } }, '+ Item'))
    );
  }
      ...
      React.createElement(FiCard, { style: { marginBottom: 12 } },
        React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 } },
          ...
          !preparoEditAberto && React.createElement('div', { style: { display: 'flex', gap: 6 } },
            ...
            o.preparo && React.createElement('button', { onClick: function () { setConfirmApagarPreparo(o.id); }, style: { background: 'none', border: 'none', fontSize: 14, cursor: 'pointer' } }, '🗑️')
          )
        ),
        preparoEditAberto
          ? React.createElement(React.Fragment, null,
      ...
      React.createElement(FiCard, { style: { marginBottom: 12 } },
        calc.itens.map(function (it, i) {
          ...
          if (receitaModo === 'porcoes' && it.unidades != null && it.unidade_nome) {
            linha = fiFmtUnidades(it.unidades) + ' ' + it.unidade_nome + ' de ' + it.nome;
          } else {
  ...
  function renderAnelRefeicao(r, kcalRefeicao, cor) {
    var pctClamped = Math.max(0, Math.min(100, Number(r.pct) || 0));
    var R = 18, C = 2 * Math.PI * R, arco = (pctClamped / 100) * C;
    return React.createElement('div', { className: 'fi-anel-refeicao-wrap' },
      React.createElement('div', { className: 'fi-anel-refeicao' },
        React.createElement('svg', { viewBox: '0 0 44 44', className: 'fi-anel-refeicao-svg' },
          React.createElement('circle', { cx: 22, cy: 22, r: R, fill: 'none', stroke: cor, strokeOpacity: 0.15, strokeWidth: 5 }),
          React.createElement('circle', { cx: 22, cy: 22, r: R, fill: 'none', stroke: cor, strokeWidth: 5, strokeDasharray: arco + ' ' + C, strokeLinecap: 'round', transform: 'rotate(-90 22 22)' })
        ),
        React.createElement('div', { className: 'fi-anel-refeicao-num' }, kcalRefeicao ? Math.round(kcalRefeicao) : '—')
      ),
      React.createElement('span', { className: 'fi-anel-refeicao-pct' }, r.pct + '%')
    );
  }
    ...
    return React.createElement('div', { style: { padding: 16 } },
      ...
      ordenadas.length > 1 && React.createElement('div', { className: 'fi-plano-chips' },
        ordenadas.map(function (r, i) {
          ...
          var corChip = fiCorRefeicao(i);
          return React.createElement('button', {
            key: r.id, className: 'fi-chip',
            style: chipAtivo ? { background: corChip, borderColor: corChip, color: '#fff' } : { borderColor: corChip },
      ...
      React.createElement('div', { className: 'fi-plano-fila', ref: filaRef },
        ordenadas.map(function (r, ri) {
          ...
          var corRefeicao = fiCorRefeicao(ri);
          return React.createElement('div', {
            ...
            React.createElement('div', { className: 'fi-refeicao-corpo' },
              React.createElement('div', { className: 'fi-refeicao-barra', style: { background: corRefeicao } }),
              React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8 } },
                refEditandoId === r.id
                  ...
                  : React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 } },
                      React.createElement('span', { className: 'fi-refeicao-num', style: { background: corRefeicao } }, ri + 1),
                      React.createElement('span', { style: { fontWeight: 900, fontSize: 16 } }, r.nome)
                    ),
                refEditandoId !== r.id && React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, flex: 'none' } },
                  renderAnelRefeicao(r, kcalRefeicao, corRefeicao),
  ...
  // ══════════════════════════════════════════════════════════════
  // PROGRESSO — avaliações, gráfico de peso, comparar (Fase 3)
  // ══════════════════════════════════════════════════════════════
  function abrirNovaAvaliacao() { setAvalEditandoId(null); setAvalConflito(null); setAvalForm(fiAvaliacaoVazia()); setAvalFormAberto(true); }
  function abrirEditarAvaliacao(a) {
    setAvalEditandoId(a.id); setAvalConflito(null);
    setAvalForm({
      data: a.data, peso: String(a.peso),
      cintura: a.cintura != null ? String(a.cintura) : '', peito: a.peito != null ? String(a.peito) : '',
      anca: a.anca != null ? String(a.anca) : '', braco: a.braco != null ? String(a.braco) : '',
      coxa: a.coxa != null ? String(a.coxa) : '', gordura_pct: a.gordura_pct != null ? String(a.gordura_pct) : '',
      notas: a.notas || ''
    });
    setAvalFormAberto(true);
  }
  function avalCampo(nome, valor) {
    setAvalForm(function (f) { var n = {}; n[nome] = valor; return Object.assign({}, f, n); });
    if (nome === 'data') setAvalConflito(null);
  }
  function fiAvaliacaoPayload(f) {
    return {
      data: f.data, peso: parseFloat(f.peso),
      cintura: f.cintura !== '' ? parseFloat(f.cintura) : null,
      peito: f.peito !== '' ? parseFloat(f.peito) : null,
      anca: f.anca !== '' ? parseFloat(f.anca) : null,
      braco: f.braco !== '' ? parseFloat(f.braco) : null,
      coxa: f.coxa !== '' ? parseFloat(f.coxa) : null,
      gordura_pct: f.gordura_pct !== '' ? parseFloat(f.gordura_pct) : null,
  ...
  function guardarAvaliacao() {
    ...
    var existente = avaliacoes.filter(function (a) { return a.data === avalForm.data && a.id !== avalEditandoId; })[0];
    if (existente) { setAvalConflito(existente); return; }
    setAvalSaving(true);
    var payload = fiAvaliacaoPayload(avalForm);
    var query = avalEditandoId
      ? db.from('fitness_avaliacoes').update(payload).eq('id', avalEditandoId)
      : db.from('fitness_avaliacoes').insert(payload);
    query.then(function (res) {
      setAvalSaving(false);
  ...
  function confirmarSubstituirAvaliacao() {
    if (!avalConflito) return;
    setAvalSaving(true);
    var payload = fiAvaliacaoPayload(avalForm);
    db.from('fitness_avaliacoes').update(payload).eq('id', avalConflito.id).then(function (res) {
      setAvalSaving(false);
  ...
  function apagarAvaliacao(id) {
    db.from('fitness_avaliacoes').delete().eq('id', id).then(function (res) {
      if (res.error) { console.error('[fitness] apagar avaliação:', res.error); window.mostrarErro('Fitness', res.error); return; }
      setConfirmApagarAvaliacao(null);
      carregarAvaliacoes();
      carregar();
    }).catch(function (e) { console.error('[fitness] apagar avaliação:', e); window.mostrarErro('Fitness', e); });
  }

  function renderResumoProgresso() {
    var n = avaliacoes.length;
    var atual = avaliacoes[n - 1];
    var anterior = n > 1 ? avaliacoes[n - 2] : null;
    var primeira = avaliacoes[0];
    var pesoMeta = perfil && perfil.peso_meta ? Number(perfil.peso_meta) : null;
    var varAnterior = anterior ? Math.round((atual.peso - anterior.peso) * 10) / 10 : null;
    var varTotal = (primeira && primeira.id !== atual.id) ? Math.round((atual.peso - primeira.peso) * 10) / 10 : null;
    var corVar = function (v) { return v == null ? 'var(--fi-texto2)' : (v <= 0 ? 'var(--fi-verde)' : 'var(--fi-amarelo)'); };
    var faltaMeta = pesoMeta ? Math.round((atual.peso - pesoMeta) * 10) / 10 : null;
    var pctBarra = null;
    if (pesoMeta && primeira && primeira.peso !== pesoMeta) {
      pctBarra = Math.max(0, Math.min(100, ((primeira.peso - atual.peso) / (primeira.peso - pesoMeta)) * 100));
    }
    return React.createElement(FiCard, { style: { marginBottom: 12, textAlign: 'center' } },
      React.createElement('div', { style: { fontSize: 34, fontWeight: 900 } }, atual.peso + ' kg'),
      anterior && React.createElement('div', { style: { fontSize: 13, fontWeight: 700, color: corVar(varAnterior), marginTop: 4 } },
        (varAnterior > 0 ? '+' : '') + varAnterior + ' kg desde ' + fiFmtDataCurtaSemAno(anterior.data)
      ),
      varTotal != null && React.createElement('div', { style: { fontSize: 12, color: 'var(--fi-texto2)', marginTop: 6 } },
        'Desde o início: ' + (varTotal > 0 ? '+' : '') + varTotal + ' kg'
      ),
      pesoMeta && React.createElement('div', { style: { marginTop: 14 } },
        React.createElement('div', { style: { fontSize: 12, color: 'var(--fi-texto2)', marginBottom: 6 } },
          faltaMeta > 0 ? ('Faltam ' + faltaMeta + ' kg para a meta (' + pesoMeta + ' kg)') : (faltaMeta < 0 ? ('Já passaste a meta em ' + Math.abs(faltaMeta) + ' kg') : 'Atingiste a meta! 🎉')
        ),
        pctBarra != null && React.createElement('div', { className: 'fi-progresso-barra-fundo' },
          React.createElement('div', { className: 'fi-progresso-barra-cheia', style: { width: pctBarra + '%' } })
        )
      )
    );
  }
  ...
  function renderGraficoPeso() {
    var n = avaliacoes.length;
    var pesos = avaliacoes.map(function (a) { return a.peso; });
    var min = Math.min.apply(null, pesos);
    var max = Math.max.apply(null, pesos);
    var pesoMeta = perfil && perfil.peso_meta ? Number(perfil.peso_meta) : null;
    if (pesoMeta != null) { min = Math.min(min, pesoMeta); max = Math.max(max, pesoMeta); }
    var folga = Math.max(0.5, (max - min) * 0.1);
    var yMin = min - folga, yMax = max + folga;
    var W = Math.max(n * 32, 280), H = 160;
    var padL = 34, padR = 12, padT = 14, padB = 26;
    var plotW = W - padL - padR, plotH = H - padT - padB;
    function xAt(i) { return n > 1 ? padL + (i / (n - 1)) * plotW : padL + plotW / 2; }
    function yAt(peso) { return padT + (1 - (peso - yMin) / (yMax - yMin)) * plotH; }
    var pontos = avaliacoes.map(function (a, i) { return { x: xAt(i), y: yAt(a.peso), a: a }; });
    var pathD = pontos.map(function (p, i) { return (i === 0 ? 'M' : 'L') + p.x.toFixed(1) + ' ' + p.y.toFixed(1); }).join(' ');
    var passoLabel = Math.max(1, Math.ceil(n / 6));
    var selecionada = graficoSelecionado ? avaliacoes.filter(function (a) { return a.id === graficoSelecionado; })[0] : null;
    return React.createElement('div', null,
      React.createElement('div', { className: 'fi-grafico-scroll' },
        React.createElement('svg', { viewBox: '0 0 ' + W + ' ' + H, style: { width: 'max(100%, ' + W + 'px)', height: H, display: 'block' } },
          pesoMeta != null && React.createElement('line', {
            x1: padL, x2: W - padR, y1: yAt(pesoMeta), y2: yAt(pesoMeta),
            stroke: 'var(--fi-texto2)', strokeWidth: 1, strokeDasharray: '4 3'
          }),
          React.createElement('text', { x: 2, y: padT + 4, fontSize: 9, fill: 'var(--fi-texto2)' }, Math.round(yMax * 10) / 10),
          React.createElement('text', { x: 2, y: H - padB, fontSize: 9, fill: 'var(--fi-texto2)' }, Math.round(yMin * 10) / 10),
          React.createElement('path', { d: pathD, fill: 'none', stroke: FI_COR, strokeWidth: 2 }),
          pontos.map(function (p, i) {
            return React.createElement(React.Fragment, { key: p.a.id },
              React.createElement('circle', {
                cx: p.x, cy: p.y, r: 4, fill: FI_COR, style: { cursor: 'pointer' },
                onClick: function () { setGraficoSelecionado(p.a.id === graficoSelecionado ? null : p.a.id); }
              }),
              i % passoLabel === 0 && React.createElement('text', { x: p.x, y: H - 6, fontSize: 9, fill: 'var(--fi-texto2)', textAnchor: 'middle' }, fiFmtDataCurtaSemAno(p.a.data))
            );
          })
        )
      ),
      selecionada && React.createElement('p', { style: { fontSize: 12, textAlign: 'center', marginTop: 6, fontWeight: 700 } }, fiFmtDataCurta(selecionada.data) + ' · ' + selecionada.peso + ' kg')
    );
  }

  function renderComparar() {
    if (avaliacoes.length < 2) return null;
    var compA = compararA || avaliacoes[0].data;
    var compB = compararB || avaliacoes[avaliacoes.length - 1].data;
    var a = avaliacoes.filter(function (x) { return x.data === compA; })[0] || avaliacoes[0];
    var b = avaliacoes.filter(function (x) { return x.data === compB; })[0] || avaliacoes[avaliacoes.length - 1];
    var campos = [['peso', 'Peso', 'kg'], ['cintura', 'Cintura', 'cm'], ['peito', 'Peito', 'cm'], ['anca', 'Anca', 'cm'], ['braco', 'Braço', 'cm'], ['coxa', 'Coxa', 'cm'], ['gordura_pct', 'Gordura', '%']];
    return React.createElement(FiCard, { style: { marginBottom: 12 } },
      React.createElement(FiLabel, null, 'Comparar'),
      React.createElement('div', { style: { display: 'flex', gap: 8, marginBottom: 10 } },
        React.createElement('select', { className: 'fi-input', value: compA, onChange: function (e) { setCompararA(e.target.value); } },
          avaliacoes.map(function (x) { return React.createElement('option', { key: x.id, value: x.data }, fiFmtDataCurta(x.data)); })
        ),
        React.createElement('select', { className: 'fi-input', value: compB, onChange: function (e) { setCompararB(e.target.value); } },
          avaliacoes.map(function (x) { return React.createElement('option', { key: x.id, value: x.data }, fiFmtDataCurta(x.data)); })
        )
      ),
      React.createElement('div', { className: 'fi-comparar-tabela' },
        React.createElement('div', { className: 'fi-comparar-linha fi-comparar-cabecalho' },
          React.createElement('span', null, ''),
          React.createElement('span', null, fiFmtDataCurta(compA)),
          React.createElement('span', null, fiFmtDataCurta(compB)),
          React.createElement('span', null, 'Dif.')
        ),
        campos.map(function (c) {
          var va = a[c[0]], vb = b[c[0]];
          if (va == null && vb == null) return null;
          var dif = (va != null && vb != null) ? Math.round((vb - va) * 10) / 10 : null;
          return React.createElement('div', { key: c[0], className: 'fi-comparar-linha' },
            React.createElement('span', null, c[1]),
            React.createElement('span', null, va != null ? (va + ' ' + c[2]) : '—'),
            React.createElement('span', null, vb != null ? (vb + ' ' + c[2]) : '—'),
            React.createElement('span', { style: { color: dif == null ? 'var(--fi-texto2)' : (dif <= 0 ? 'var(--fi-verde)' : 'var(--fi-amarelo)'), fontWeight: 700 } },
              dif != null ? ((dif > 0 ? '+' : '') + dif) : '—'
            )
          );
        })
  ...
  function renderFormAvaliacao() {
    var f = avalForm;
    return React.createElement(FiCard, { style: { marginBottom: 12 } },
      React.createElement(FiLabel, null, avalEditandoId ? 'Editar avaliação' : '+ Nova avaliação'),
      avalConflito && React.createElement('p', { style: { fontSize: 12, color: 'var(--fi-amarelo)', fontWeight: 700, marginBottom: 8 } },
        '⚠️ Já existe uma avaliação em ' + fiFmtDataCurta(avalConflito.data) + '. Guardar substitui os valores dessa em vez de criar outra.'
      ),
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 } },
        React.createElement('div', null,
          React.createElement(FiLabel, { style: { margin: '0 0 4px' } }, 'Data'),
          React.createElement('input', { type: 'date', className: 'fi-input', autoComplete: 'off', value: f.data, onChange: function (e) { avalCampo('data', e.target.value); } })
        ),
        React.createElement('div', null,
          React.createElement(FiLabel, { style: { margin: '0 0 4px' } }, 'Peso (kg)'),
          React.createElement('input', { type: 'number', step: '0.1', className: 'fi-input', autoComplete: 'off', value: f.peso, onChange: function (e) { avalCampo('peso', e.target.value); } })
        )
      ),
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 } },
        [['cintura', 'Cintura (cm)'], ['peito', 'Peito (cm)'], ['anca', 'Anca (cm)'], ['braco', 'Braço (cm)'], ['coxa', 'Coxa (cm)'], ['gordura_pct', 'Gordura (%)']].map(function (c) {
          return React.createElement('div', { key: c[0] },
            React.createElement(FiLabel, { style: { margin: '0 0 4px' } }, c[1]),
            React.createElement('input', { type: 'number', step: '0.1', className: 'fi-input', autoComplete: 'off', value: f[c[0]], onChange: function (e) { avalCampo(c[0], e.target.value); } })
          );
        })
      ),
      React.createElement('input', { type: 'text', className: 'fi-input', autoComplete: 'off', placeholder: 'Notas (opcional)', value: f.notas, onChange: function (e) { avalCampo('notas', e.target.value); }, style: { marginBottom: 10 } }),
      React.createElement('div', { style: { display: 'flex', gap: 8 } },
        React.createElement('button', { className: 'fi-btn', style: { flex: 1 }, onClick: function () { setAvalFormAberto(false); setAvalEditandoId(null); setAvalConflito(null); } }, 'Cancelar'),
        avalConflito
          ? React.createElement('button', { className: 'fi-btn fi-btn-ativo', style: { flex: 1 }, disabled: avalSaving, onClick: confirmarSubstituirAvaliacao }, avalSaving ? 'A guardar…' : '✓ Substituir')
          : React.createElement('button', { className: 'fi-btn fi-btn-ativo', style: { flex: 1 }, disabled: avalSaving || !f.peso, onClick: guardarAvaliacao }, avalSaving ? 'A guardar…' : '✓ Guardar')
      )
    );
  }

  function renderListaAvaliacoes() {
    var desc = avaliacoes.slice().reverse();
    return React.createElement('div', null,
      desc.map(function (a, i) {
        var anterior = desc[i + 1];
        var varPeso = anterior ? Math.round((a.peso - anterior.peso) * 10) / 10 : null;
        var medidas = [];
        if (a.cintura != null) medidas.push('Cintura ' + a.cintura + ' cm');
        if (a.peito != null) medidas.push('Peito ' + a.peito + ' cm');
        if (a.anca != null) medidas.push('Anca ' + a.anca + ' cm');
        if (a.braco != null) medidas.push('Braço ' + a.braco + ' cm');
        if (a.coxa != null) medidas.push('Coxa ' + a.coxa + ' cm');
        if (a.gordura_pct != null) medidas.push('Gordura ' + a.gordura_pct + '%');
        return React.createElement(FiCard, { key: a.id, style: { marginBottom: 8 } },
          React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
            React.createElement('div', { style: { minWidth: 0 } },
              React.createElement('div', { style: { fontWeight: 800, fontSize: 14 } }, fiFmtDataCurta(a.data)),
              React.createElement('div', { style: { fontSize: 13, marginTop: 2 } },
                a.peso + ' kg',
                varPeso != null && React.createElement('span', { style: { marginLeft: 8, fontSize: 12, fontWeight: 700, color: varPeso <= 0 ? 'var(--fi-verde)' : 'var(--fi-amarelo)' } }, (varPeso > 0 ? '+' : '') + varPeso + ' kg')
              ),
              medidas.length > 0 && React.createElement('div', { style: { fontSize: 11.5, color: 'var(--fi-texto2)', marginTop: 4 } }, medidas.join(' · ')),
              a.notas && React.createElement('div', { style: { fontSize: 11.5, color: 'var(--fi-texto2)', marginTop: 4, fontStyle: 'italic' } }, a.notas)
            ),
            React.createElement('div', { style: { display: 'flex', gap: 4, flex: 'none' } },
              React.createElement('button', { onClick: function () { abrirEditarAvaliacao(a); }, style: { background: 'none', border: 'none', fontSize: 15, cursor: 'pointer' } }, '✏️'),
              React.createElement('button', { onClick: function () { setConfirmApagarAvaliacao(a.id); }, style: { background: 'none', border: 'none', fontSize: 15, cursor: 'pointer' } }, '🗑️')
            )
          )
        );
      })
  ...
  function renderProgresso() {
    if (avaliacoesCarregando && !avaliacoes.length) {
      return React.createElement('div', { style: { padding: 16 } },
    ...
    if (!avaliacoes.length) {
      return React.createElement('div', { style: { padding: 16 } },
        React.createElement(FiCard, { style: { marginBottom: 12, textAlign: 'center' } },
          React.createElement('p', { style: { fontSize: 13, color: 'var(--fi-texto2)' } }, 'Ainda não tens avaliações registadas. Regista a primeira para começares a acompanhar o teu progresso.')
        ),
        renderFormAvaliacao()
      );
    }
    return React.createElement('div', { style: { padding: 16 } },
      ...
      React.createElement(FiCard, { style: { marginBottom: 12 } }, renderGraficoPeso()),
      renderComparar(),
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 } },
        React.createElement(FiLabel, { style: { margin: 0 } }, 'Histórico'),
        !avalFormAberto && React.createElement('button', { className: 'fi-btn fi-btn-ativo', style: { padding: '6px 12px', fontSize: 12 }, onClick: abrirNovaAvaliacao }, '+ Nova avaliação')
      ),
      avalFormAberto && renderFormAvaliacao(),
  ...
  var modalApagarPreparo = confirmApagarPreparo && React.createElement('div', {
    style: { position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 20 },
    onClick: function (e) { if (e.target === e.currentTarget) setConfirmApagarPreparo(null); }
  },
    React.createElement(FiCard, { style: { maxWidth: 340, width: '100%' } },
      React.createElement('p', { style: { fontWeight: 700, marginBottom: 16 } }, 'Apagar o «Como se faz» desta receita?'),
      React.createElement('div', { style: { display: 'flex', gap: 10 } },
        React.createElement('button', { className: 'fi-btn', style: { flex: 1 }, onClick: function () { setConfirmApagarPreparo(null); } }, 'Cancelar'),
        React.createElement('button', { className: 'fi-btn fi-btn-perigo', style: { flex: 1 }, onClick: function () { apagarPreparo(confirmApagarPreparo); } }, 'Apagar')
      )
    )
  );
  ...
  var modalApagarAvaliacao = confirmApagarAvaliacao && React.createElement('div', {
    style: { position: 'fixed', inset: 0, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 20 },
    onClick: function (e) { if (e.target === e.currentTarget) setConfirmApagarAvaliacao(null); }
  },
    React.createElement(FiCard, { style: { maxWidth: 340, width: '100%' } },
      React.createElement('p', { style: { fontWeight: 700, marginBottom: 16 } }, 'Apagar esta avaliação?'),
      React.createElement('div', { style: { display: 'flex', gap: 10 } },
        React.createElement('button', { className: 'fi-btn', style: { flex: 1 }, onClick: function () { setConfirmApagarAvaliacao(null); } }, 'Cancelar'),
        React.createElement('button', { className: 'fi-btn fi-btn-perigo', style: { flex: 1 }, onClick: function () { apagarAvaliacao(confirmApagarAvaliacao); } }, 'Apagar')
      )
    )
  );
