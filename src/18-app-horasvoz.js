...
// Quadro "A minha semana" — resumo grande no topo do ecrã Dia e da aba
// Semana (mesmo componente nos dois sítios). Só apresenta números já
// produzidos noutro lado (weekCalc.meta_semana, dias[].total/meta via
// hvDiasSemanaCompleta/hvDiasDeSemana, hvResumoAte) — nunca recalcula
// meta/total por si. A repartição do que falta pelos dias por registar
// usa o mesmo método do maior resto (hvCompensarExtras) já usado para
// os extras por dia, para os números baterem sempre certos.
// p: { mondayStr, dias, hoje, contarDesde, metaSemana, percentagem, diaNormalMin, onClick }
function hvQuadroSemanaPropsIguais(a, b) {
  if (a.mondayStr !== b.mondayStr || a.hoje !== b.hoje || a.contarDesde !== b.contarDesde ||
    a.metaSemana !== b.metaSemana || a.percentagem !== b.percentagem || a.diaNormalMin !== b.diaNormalMin) return false;
  if (a.dias.length !== b.dias.length) return false;
  for (var i = 0; i < a.dias.length; i++) {
    var x = a.dias[i], y = b.dias[i];
    if (x.dateStr !== y.dateStr || x.total !== y.total || x.meta !== y.meta || x.temRegisto !== y.temRegisto ||
      x.livre !== y.livre || x.fimDeSemana !== y.fimDeSemana || x.isoDow !== y.isoDow) return false;
  }
  return true;
}
var HvQuadroSemana = React.memo(function HvQuadroSemana(p) {
  var calc = React.useMemo(function () {
    var feitoH = p.dias.reduce(function (s, d) { return s + d.total; }, 0);
    var metaH = p.metaSemana;
    var faltaH = Math.max(0, metaH - feitoH);
    var aMais = feitoH >= metaH;
    var pct = metaH > 0 ? (feitoH / metaH * 100) : 0;
    var extraAteHoje = hvResumoAte(p.dias, p.hoje, p.contarDesde).extra;
    var diasFaltam = p.dias.filter(function (d) { return !d.fimDeSemana && !d.livre && !d.temRegisto; });
    var faltaLinha = null;
    if (diasFaltam.length && faltaH > 0) {
      var itens = diasFaltam.map(function (d) { return { dateStr: d.dateStr, valor: (faltaH * 60) / diasFaltam.length }; });
      var comp = hvCompensarExtras(itens);
      var primeiro = diasFaltam[0], ultimo = diasFaltam[diasFaltam.length - 1];
      var rotuloDias = diasFaltam.length === 1 ? HV_DIA_CURTO[primeiro.isoDow - 1] : HV_DIA_CURTO[primeiro.isoDow - 1] + '–' + HV_DIA_CURTO[ultimo.isoDow - 1];
      faltaLinha = 'Falta ' + hvMinToHM(faltaH * 60) + ' → ' + rotuloDias + ' ' + hvMinToHM(comp[primeiro.dateStr]) + (diasFaltam.length > 1 ? ' por dia' : '');
    }
    var previsaoDiffMin = diasFaltam.length ? Math.round(feitoH * 60 + diasFaltam.length * p.diaNormalMin - metaH * 60) : null;
    return { feitoH: feitoH, metaH: metaH, faltaH: faltaH, aMais: aMais, pct: pct, extraAteHoje: extraAteHoje, faltaLinha: faltaLinha, temDiasFaltam: diasFaltam.length > 0, previsaoDiffMin: previsaoDiffMin };
  }, [p.dias, p.hoje, p.contarDesde, p.metaSemana, p.diaNormalMin]);
  var kw = hvKw(hvMk(p.mondayStr));
  var corBarra = calc.aMais ? 'var(--hv-positivo)' : 'var(--hv-principal)';
  var escalaMax = Math.max(100, calc.pct);
  var largura = Math.min(100, calc.pct / escalaMax * 100);
  var marcaPos = escalaMax > 100 ? (100 / escalaMax * 100) : null;
  return React.createElement('div', {
    className: 'hv-card', onClick: p.onClick,
    style: { cursor: p.onClick ? 'pointer' : 'default', display: 'flex', flexDirection: 'column', gap: 8 }
  },
    React.createElement('div', { style: HV_ESTILO.etiquetaSm }, 'Semana KW ' + kw + ' · ' + p.percentagem + '%'),
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 8 } },
      React.createElement('div', { style: { textAlign: 'left' } },
        React.createElement('div', { style: HV_ESTILO.etiquetaSm }, 'Meta semana'),
        React.createElement('div', { style: HV_ESTILO.totalNumero }, hvMinToHM(calc.metaH * 60))
      ),
      React.createElement('div', { style: { textAlign: 'center' } },
        React.createElement('div', { style: HV_ESTILO.etiquetaSm }, 'Feito'),
        React.createElement('div', { style: HV_ESTILO.totalNumero }, hvMinToHM(calc.feitoH * 60))
      ),
      React.createElement('div', { style: { textAlign: 'right' } },
        React.createElement('div', { style: HV_ESTILO.etiquetaSm }, calc.aMais ? 'A mais' : 'Falta'),
        React.createElement('div', { style: calc.aMais ? Object.assign({}, HV_ESTILO.totalNumero, { color: 'var(--hv-positivo)' }) : HV_ESTILO.totalNumero },
          (calc.aMais ? '+' : '') + hvMinToHM((calc.aMais ? calc.feitoH - calc.metaH : calc.faltaH) * 60)
        )
      )
    ),
    React.createElement('div', { style: { position: 'relative', height: 8, borderRadius: 4, background: 'var(--hv-borda)', overflow: 'hidden' } },
      React.createElement('div', { style: { position: 'absolute', left: 0, top: 0, bottom: 0, width: largura + '%', background: corBarra, borderRadius: 4 } }),
      marcaPos != null && React.createElement('div', { style: { position: 'absolute', left: marcaPos + '%', top: 0, bottom: 0, width: 2, background: 'var(--hv-fundo)' } })
    ),
    calc.faltaLinha && React.createElement('div', { style: { fontSize: 12, color: 'var(--hv-texto2)' } }, calc.faltaLinha),
    !calc.temDiasFaltam && React.createElement('div', { style: { fontSize: 12, color: 'var(--hv-texto2)' } }, 'Semana fechada'),
    calc.temDiasFaltam && calc.previsaoDiffMin != null && React.createElement('div', { style: { fontSize: 12, color: 'var(--hv-texto2)' } },
      'Se fizeres dia normal (' + hvMinToHM(p.diaNormalMin) + ') nos dias que faltam: fim da semana ',
      React.createElement(HvSaldoTextoMin, { valorMin: calc.previsaoDiffMin })
    ),
    React.createElement('div', { style: { fontSize: 11, color: 'var(--hv-texto2)' } }, 'Extra até hoje ', React.createElement(HvSaldoTexto, { valor: calc.extraAteHoje }))
  );
}, hvQuadroSemanaPropsIguais);
...
function HorasVozApp(props) {
  ...
  // Duração de um "dia normal" segundo o horário habitual da config da
  // semana em vista (menos a pausa) — usada só na previsão do quadro
  // "A minha semana", mesma fórmula já usada em renderModoA (com cfgHoje).
  var diaNormalMinSemana = hvBlocoMin(cfgSemana.manha_inicio, cfgSemana.manha_fim) + hvBlocoMin(cfgSemana.tarde_inicio, cfgSemana.tarde_fim) - (cfgSemana.pausa_min || 0);
  ...
  function renderFaixaSemana() {
    ...
    return React.createElement('div', null,
      React.createElement(HvQuadroSemana, {
        mondayStr: mondayCur, dias: dias, hoje: hoje, contarDesde: contarDesde, metaSemana: weekCalc.meta_semana,
        percentagem: cfgSemana.percentagem, diaNormalMin: diaNormalMinSemana, onClick: function () { setView('semana'); }
      }),
      React.createElement('div', { className: 'hv-faixa', style: { marginTop: 10 } }, bolinhas)
    );
  }
  ...
  function renderSemana() {
    ...
    return React.createElement('div', { style: HV_ESTILO.pagina },
      ...
      React.createElement(HvQuadroSemana, {
        mondayStr: mondayCur, dias: dias, hoje: hoje, contarDesde: contarDesde, metaSemana: weekCalc.meta_semana,
        percentagem: cfgSemana.percentagem, diaNormalMin: diaNormalMinSemana
      }),
