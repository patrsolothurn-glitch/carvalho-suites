...
function vzCatLabel(key) {
  var k = vzNormCat(key);
  for (var i = 0; i < VZ_CATEGORIAS.length; i++) { if (VZ_CATEGORIAS[i].key === k) return VZ_CATEGORIAS[i].label; }
  return VZ_CAT_DEFAULT;
}
var VZ_CAT_KEY = 'vz_categoria';
function vzLoadCat() {
  try { return vzNormCat(localStorage.getItem(VZ_CAT_KEY)); } catch (e) { return VZ_CAT_DEFAULT; }
}
function vzSaveCat(k) {
  try { localStorage.setItem(VZ_CAT_KEY, vzNormCat(k)); } catch (e) {}
}
// Opções de pasta agrupadas por categoria (usadas nos dois <select>)
function vzPastaOptions(pastas) {
  return VZ_CATEGORIAS.map(function(cat) {
    var lista = pastas.filter(function(pa) { return vzNormCat(pa.categoria) === cat.key; });
    if (!lista.length) return null;
    return React.createElement('optgroup', { key: cat.key, label: cat.label },
      lista.map(function(pa) { return React.createElement('option', { key: pa.id, value: pa.id }, pa.titulo); })
    );
  });
}
    ...
    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
      ...
      React.createElement('div', { style: { flex: 1, minWidth: 0 } },
        ...
        React.createElement('div', { style: { fontSize: 11, color: T.muted, marginTop: 2 } },
          vzFmtDateTime(vozDataEfetiva(g)) + ' · ' + vzFmtDuration(g.duracao_seg) + (arquivada ? ' · no Drive' : ''),
          g.data_ref && React.createElement('span', { title: 'Data ajustada manualmente', style: { marginLeft: 4, color: T.muted } }, '•')
        ),
...
// data/hora efetiva de uma gravação — data_ref (se definida) senão gravado_em
function vozDataEfetiva(g) {
  return g.data_ref || g.gravado_em;
}

// valor para o input datetime-local (hora local, sem segundos)
function vozParaInput(ts) {
  var d = new Date(ts);
...
// gravações sempre ordenadas pela data efetiva, mais recente primeiro
function vozOrdenarGravacoes(lista) {
  return lista.slice().sort(function(a, b) {
    return new Date(vozDataEfetiva(b)) - new Date(vozDataEfetiva(a));
  });
}

// KW (número) de um título de pasta tipo "KW 37" / "Kw37" — null se não for esse padrão
function vozExtrairKw(titulo) {
  var m = vozNorm(titulo).match(/^kw(\d+)$/);
...
function vozAgruparPorDia(lista) {
  ...
  lista.forEach(function(g) {
    var k = vozDiaKey(vozDataEfetiva(g));
    if (!mapa[k]) { mapa[k] = []; ordem.push(k); }
    mapa[k].push(g);
  });
  return ordem.map(function(k) {
    return { key: k, label: vozDiaLabel(vozDataEfetiva(mapa[k][0])), itens: mapa[k] };
  });
}
...
var VzCategoriaTabs = function VzCategoriaTabs(p) {
  return React.createElement('div', { style: { display: 'flex', gap: 6 } },
    VZ_CATEGORIAS.map(function(cat) {
      ...
      return React.createElement('button', {
        key: cat.key,
        onClick: function() { p.onChange(cat.key); },
        style: {
...
var VzRecorderTab = function VzRecorderTab(p) {
  ...
  if (p.recordedBlob) {
    return React.createElement('div', { style: { padding: 16 } },
      ...
      React.createElement('div', { style: { fontSize: 12, color: T.muted, fontWeight: 700, marginBottom: 6, textTransform: 'uppercase' } }, 'Data e hora'),
      React.createElement('input', {
        type: 'datetime-local', autoComplete: 'off', value: p.saveDataRef,
        onChange: function(e) { p.onDataRefChange(e.target.value); },
        style: { width: '100%', background: T.surface2, border: '1px solid ' + T.border, borderRadius: 10, padding: '12px 14px', color: T.text, fontSize: 15, marginBottom: 14, boxSizing: 'border-box' }
      }),
  ...
  return React.createElement('div', null,
    ...
    !p.loading && React.createElement('div', { style: { marginTop: 24, borderTop: '1px solid ' + T.border, paddingTop: 16 } },
      React.createElement('div', { style: { fontSize: 12, fontWeight: 800, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 } }, 'Pastas · ' + vzCatLabel(p.categoria)),
      p.pastasCat.length === 0 && React.createElement('div', { style: { color: T.muted, fontSize: 13, padding: '4px 0' } }, 'Ainda não há pastas em ' + vzCatLabel(p.categoria) + '.'),
      p.pastasCat.map(function(pa) {
      ...
      React.createElement('div', { style: { display: 'flex', gap: 8, marginTop: 10 } },
        React.createElement('input', {
          type: 'text', autoComplete: 'off', placeholder: 'Nova pasta em ' + vzCatLabel(p.categoria) + '…', value: p.novaPastaNome,
...
function VozApp(props) {
  ...
  var _s34 = React.useState(function() { return vzLoadCat(); }); var categoria = _s34[0], setCategoriaState = _s34[1];
  function selecionarCategoria(k) { var c = vzNormCat(k); setCategoriaState(c); vzSaveCat(c); }
  ...
  var _s35 = React.useState(''); var saveDataRef = _s35[0], setSaveDataRef = _s35[1];
  var _s36 = React.useState(false); var saveDataRefManual = _s36[0], setSaveDataRefManual = _s36[1];
  var _s37 = React.useState(null); var kwConfirm = _s37[0], setKwConfirm = _s37[1];
  ...
  function carregar() {
    ...
    Promise.all([
      ...
      setPastas((pRes.data || []).map(function(pa) { return Object.assign({}, pa, { categoria: vzNormCat(pa.categoria) }); }));
      setGravacoes(vozOrdenarGravacoes(gRes.data || []));
      setLoading(false);
    }).catch(function(e) {
  ...
  function iniciarGravacao() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
      ...
      rec.onstop = function() {
        ...
        setSaveTitulo('');
        setSavePastaId(null);
        setSaveDataRef(vozParaInput(new Date()));
        setSaveDataRefManual(false);
  ...
  function guardarGravacao() {
    ...
    var dataRefISO = (saveDataRefManual && saveDataRef) ? new Date(saveDataRef).toISOString() : null;

    function prosseguir(pastaIdFinal) {
      setSaving(true);
      setErro(null);
      ...
      vzUploadRecording(db, profile.id, recordedBlob, recordedMime).then(function(path) {
        ...
        if (res.error) throw res.error;
        var novaLinha = (res.data && res.data[0]) || { id: 'tmp-' + Date.now(), pasta_id: pastaIdFinal, titulo: titulo, gravado_em: gravadoEm, data_ref: dataRefISO, duracao_seg: recordedDurationSec, mime_type: recordedMime, storage_path: uploadedPath };
        setGravacoes(function(prev) { return vozOrdenarGravacoes([novaLinha].concat(prev)); });
        descartarGravacao();
        setSaving(false);
      }).catch(function(e) {
        setSaving(false);
    ...
    if (dataRefISO) {
      vozPedirConfirmacaoKw(dataRefISO, savePastaId, categoria, prosseguir);
    } else {
      prosseguir(savePastaId);
    }
  }
  ...
  function criarPasta() {
    ...
    if (!titulo || !db) return;
    setErro(null);
    db.from('voz_pastas').insert({ titulo: titulo, categoria: categoria }).select().then(function(res) {
      if (res.error) throw res.error;
      var novas = (res.data || []).map(function(pa) { return Object.assign({}, pa, { categoria: vzNormCat(pa.categoria) }); });
      setPastas(function(prev) { return prev.concat(novas).sort(function(a, b) { return a.titulo.localeCompare(b.titulo); }); });
      setNovaPastaNome('');
  ...
  function guardarEdicaoPasta() {
    ...
    var cat = vzNormCat(editPasta.categoria);
    setErro(null);
    db.from('voz_pastas').update({ titulo: titulo, categoria: cat }).eq('id', editPasta.id).then(function(res) {
      if (res.error) throw res.error;
      setPastas(function(prev) {
        return prev.map(function(pa) { return pa.id === editPasta.id ? Object.assign({}, pa, { titulo: titulo, categoria: cat }) : pa; })
          .sort(function(a, b) { return a.titulo.localeCompare(b.titulo); });
      });
      setEditPasta(null);
      selecionarCategoria(cat);
    }).catch(function(e) { setErro('Falha ao guardar pasta: ' + (e && e.message ? e.message : e)); });
  }
  ...
  // Se a data escolhida for de outra KW, pergunta antes de prosseguir; senão chama continuar() logo.
  function vozPedirConfirmacaoKw(dataISO, pastaIdAtual, categoriaContexto, continuar) {
    var alvoKw = vozIsoWeek(new Date(dataISO));
    var pastaAtual = pastas.filter(function(pa) { return pa.id === pastaIdAtual; })[0] || null;
    var kwAtual = pastaAtual ? vozExtrairKw(pastaAtual.titulo) : null;
    if (kwAtual === alvoKw) { continuar(pastaIdAtual); return; }
    var cat = pastaAtual ? vzNormCat(pastaAtual.categoria) : categoriaContexto;
    var existente = pastas.filter(function(pa) { return vzNormCat(pa.categoria) === cat && vozExtrairKw(pa.titulo) === alvoKw; })[0] || null;
    setKwConfirm({
      alvoKw: alvoKw,
      existente: existente,
      onManter: function() { setKwConfirm(null); continuar(pastaIdAtual); },
      onMover: function() {
        if (existente) { setKwConfirm(null); continuar(existente.id); return; }
        db.from('voz_pastas').insert({ titulo: 'KW ' + alvoKw, categoria: cat }).select().then(function(res) {
          if (res.error) throw res.error;
          var nova = Object.assign({}, res.data[0], { categoria: vzNormCat(res.data[0].categoria) });
          setPastas(function(prev) { return prev.concat([nova]).sort(function(a, b) { return a.titulo.localeCompare(b.titulo); }); });
          setKwConfirm(null);
          continuar(nova.id);
        }).catch(function(e) {
          setKwConfirm(null);
          setErro('Falha ao criar pasta: ' + (e && e.message ? e.message : e));
        });
      }
    });
  }
  ...
  function guardarEdicaoGravacao() {
    ...
    setErro(null);
    var novoDataRef = editGravacao._dataRefNull ? null
      : (editGravacao._dataAlterada && editGravacao._dataInput) ? new Date(editGravacao._dataInput).toISOString()
      : editGravacao.data_ref;

    function prosseguir(pastaIdFinal) {
      db.from('voz_gravacoes').update({ titulo: titulo, pasta_id: pastaIdFinal, data_ref: novoDataRef }).eq('id', editGravacao.id).then(function(res) {
        if (res.error) throw res.error;
        setGravacoes(function(prev) { return vozOrdenarGravacoes(prev.map(function(g) { return g.id === editGravacao.id ? Object.assign({}, g, { titulo: titulo, pasta_id: pastaIdFinal, data_ref: novoDataRef }) : g; })); });
        setEditGravacao(null);
      }).catch(function(e) { setErro('Falha ao atualizar gravação: ' + (e && e.message ? e.message : e)); });
    }

    if (editGravacao._dataAlterada && novoDataRef) {
      vozPedirConfirmacaoKw(novoDataRef, editGravacao.pasta_id, categoria, prosseguir);
    } else {
      prosseguir(editGravacao.pasta_id);
    }
  }
  ...
  var pastasCat = pastas.filter(function(pa) { return vzNormCat(pa.categoria) === categoria; });

  var grupos = pastasCat.map(function(pa) {
    ...
    .filter(function(grupo) { return grupo.itens.length > 0 || grupo.key !== 'sem-pasta' || pastasCat.length === 0; });
  ...
  return React.createElement('div', { style: { minHeight: '100vh', background: T.bg, color: T.text, fontFamily: "'Inter',system-ui,sans-serif", paddingBottom: 40 } },
    ...
    React.createElement('div', { style: { padding: '16px' } },
      tab === 'gravador' && React.createElement(VzRecorderTab, {
        ...
        saveDataRef: saveDataRef, onDataRefChange: function(v) { setSaveDataRef(v); setSaveDataRefManual(true); },
        ...
        pastas: pastas, pastasCat: pastasCat, grupos: grupos, loading: loading, erro: erro,
        categoria: categoria, onCategoriaChange: selecionarCategoria,
        playingId: playingId, playingUrl: playingUrl, onPlay: tocarGravacao, onPlayEnded: function() { setPlayingId(null); setPlayingUrl(''); },
        onEditGravacao: function(g) { setEditGravacao(Object.assign({}, g, { _dataInput: vozParaInput(vozDataEfetiva(g)), _dataAlterada: false, _dataRefNull: false })); }, onDeleteGravacao: apagarGravacao,
    ...
    editGravacao && React.createElement('div', { style: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', zIndex: 50 } },
      React.createElement('div', { style: { background: T.surface, borderRadius: '18px 18px 0 0', padding: 20, width: '100%' } },
        ...
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 } },
          React.createElement('div', { style: { fontSize: 12, color: T.muted, fontWeight: 700, textTransform: 'uppercase', flex: 1 } }, 'Data e hora'),
          editGravacao.data_ref && React.createElement('button', {
            onClick: function() { setEditGravacao(Object.assign({}, editGravacao, { _dataInput: vozParaInput(editGravacao.gravado_em), _dataAlterada: false, _dataRefNull: true })); },
            style: { background: 'none', border: 'none', color: T.gold, fontWeight: 700, fontSize: 12, cursor: 'pointer', padding: 0 }
          }, 'Repor data real')
        ),
        React.createElement('input', {
          type: 'datetime-local', autoComplete: 'off', value: editGravacao._dataInput,
          onChange: function(e) { setEditGravacao(Object.assign({}, editGravacao, { _dataInput: e.target.value, _dataAlterada: true, _dataRefNull: false })); },
          style: { width: '100%', background: T.surface2, border: '1px solid ' + T.border, borderRadius: 10, padding: '12px 14px', color: T.text, fontSize: 15, marginBottom: 10, boxSizing: 'border-box' }
        }),
        React.createElement('select', {
    ...
    editPasta && React.createElement('div', { style: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', zIndex: 50 } },
      React.createElement('div', { style: { background: T.surface, borderRadius: '18px 18px 0 0', padding: 20, width: '100%' } },
        React.createElement('div', { style: { fontWeight: 800, fontSize: 16, marginBottom: 14 } }, 'Editar pasta'),
        React.createElement('input', {
        ...
        React.createElement('select', {
          value: vzNormCat(editPasta.categoria),
          onChange: function(e) { setEditPasta(Object.assign({}, editPasta, { categoria: e.target.value })); },
          style: { width: '100%', background: T.surface2, border: '1px solid ' + T.border, borderRadius: 10, padding: '12px 14px', color: T.text, fontSize: 15, marginBottom: 18, boxSizing: 'border-box' }
        }, VZ_CATEGORIAS.map(function(cat) {
          return React.createElement('option', { key: cat.key, value: cat.key }, cat.emoji + ' ' + cat.label);
        })),
        React.createElement('div', { style: { display: 'flex', gap: 10 } },
    ...
    // ── Modal: confirmar mudança de pasta KW por causa da data escolhida ──
    kwConfirm && React.createElement('div', { style: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-end', zIndex: 60 } },
      React.createElement('div', { style: { background: T.surface, borderRadius: '18px 18px 0 0', padding: 20, width: '100%' } },
        React.createElement('div', { style: { fontWeight: 800, fontSize: 16, marginBottom: 4 } },
          kwConfirm.existente
            ? 'Esta data é da KW ' + kwConfirm.alvoKw + '. Mover para a pasta Kw ' + kwConfirm.alvoKw + '?'
            : 'Não existe pasta Kw ' + kwConfirm.alvoKw + '. Criar e mover?'
        ),
        React.createElement('div', { style: { display: 'flex', gap: 10, marginTop: 16 } },
          React.createElement('button', { onClick: kwConfirm.onManter, style: { flex: 1, background: T.surface2, color: T.muted, border: 'none', borderRadius: 10, padding: 14, fontWeight: 700, cursor: 'pointer' } }, 'Manter aqui'),
          React.createElement('button', { onClick: kwConfirm.onMover, style: { flex: 1, background: '#40445A', color: '#E8E9EF', border: 'none', borderRadius: 10, padding: 14, fontWeight: 800, cursor: 'pointer' } }, kwConfirm.existente ? 'Mover' : 'Criar e mover')
        )
      )
    )
  );
